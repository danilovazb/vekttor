...
function linkOrdem($titulo,$valor,$ativo,$parametro_adicional=''){
	$link_ordem="?tela_id=".$_GET[tela_id]."&pagina=".$_GET[pagina]."&limitador=".$_GET[limitador]."&busca=".$_GET[busca]."&$parametro_adicional";
...
function unidade($unidade){
	$num_unidade = array("","um","dois","três","quatro","cinco","seis","sete","oito","nove");
...
function removeAcentos($string, $slug = false) {
   ...
   $string = preg_match("/( )/",'',$string);
	...
	$string = preg_match('[^a-z0-9\-]','',$string);
	$string = preg_match('--','-',$string);
	return $string;
}
