...
$(document).ready(function (){ 
	$("#tabela_dados tr").live("click",function(){
		...
		window.open('modulos/aluguel/relatorios/disponibilidade/form.php?id='+id,'carregador');
	});
});
</script>
<script>
...
<table cellpadding="0" cellspacing="0" width="100%" id="tabela_dados" >
	...
	$sql=mysql_query($t="SELECT * FROM aluguel_equipamentos WHERE vkt_id = '$vkt_id' $fim LIMIT ".paginacao_limite($registros,$_GET[pagina],$_GET[limitador]));
	//echo $t;
	while($e=mysql_fetch_object($sql)){		
			...
			$equipamento_itens_locados = mysql_fetch_object(mysql_query($t="SELECT COUNT(*) as qtd FROM aluguel_equipamentos_itens WHERE equipamento_id='$e->id' AND status='2' AND vkt_id='$vkt_id'"));
														//echo $t;
	?>      
    <tr id="<?=$e->id;?>">
        <td width="60"><?=$e->id;?></td>
        ...
        <td width="100"><?=$equipamento_itens_locados->qtd?></td>
    	<td></td>
    </tr>
    <?php
		}
