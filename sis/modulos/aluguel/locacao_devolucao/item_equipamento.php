				...
				$dias           = $_POST['dias'];
					...
					if(!empty($equipamento_id)){
						$item_equipamento = (mysql_query(" SELECT * FROM aluguel_equipamentos_itens WHERE equipamento_id = '$equipamento_id' AND status = '1' AND vkt_id = '$vkt_id' "));
								...
								for( $i=0;$i < $qtd;$i++){									
									$AllEquipamentos = mysql_fetch_object(mysql_query(" SELECT * FROM aluguel_equipamentos WHERE id = '".$equipamento[$i]['equipamento_id']."'"));
										$v_total = ((($dias) / ($AllEquipamentos->periodo)) * $AllEquipamentos->vlr_aluguel);
										$TotalEquip += $v_total; 
										
								echo utf8_encode('<tr id="'.$equipamento->id.'" >
										  <td style="border-left:1px solid #CCC;">
										  	<input type="hidden" name="id_equipamento[]" id="id_equipamento" value='.$equipamento[$i]['id'].' size="5">
											'.iconv('iso-8859-2','utf-8',$AllEquipamentos->descricao).' - '.$equipamento[$i]['numero_serie'].'
										  </td>
										  <td>1</td>
										  <td>'.$dias.'</td>
										  <td>
										  <input type="hidden" name="valor_item[]" id="id_item" value="'.moedaUsaToBr($AllEquipamentos->vlr_aluguel).'" style="width:50px">
											'.moedaUsaToBr($AllEquipamentos->vlr_aluguel).' / '.$AllEquipamentos->periodo.'
										  </td>
										  <td>
