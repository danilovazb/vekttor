...
if(!empty($_GET['status'])){
	mysql_query($t="UPDATE estoque_consumos SET status='".$_GET['status']."' WHERE id='".$_GET['compra_id']."'");
}
...
function novoPedido(t){
	...
	if(estoque_id!=''){
	location='?tela_id=499&pagina=<? if(empty($_GET['pagina'])){echo 1;}else{echo $_GET['pagina'];}?>&limitador=<? if(empty($_GET['pagina'])){echo 30;}else{echo $_GET['limitador'];}?>&fornecedor_id=0&estoque_id='+estoque_id;
	}else {
...
<a href="?tela_id=498" class="navegacao_ativo">
<span></span>Consumo
</a>
</div>
<div id="barra_info">
...
<input type="button" style="float:right; margin:3px 5px 0 10px;" value="Novo Consumo" onclick="novoPedido(this)" />
...
<script>resize()</script><!-- Isso é Necessário para a criação o resize -->
	...
	if(!empty($_POST['busca'])){
		$compras_q = mysql_query($t="SELECT * FROM estoque_consumos WHERE id LIKE '".$_POST['busca']."'");
		//echo $t;
	}else{
		$registros= mysql_result(mysql_query($t="SELECT count(*) FROM estoque_consumos WHERE vkt_id='$vkt_id' $fim ORDER BY id DESC"),0,0);
		$compras_q = mysql_query($t="SELECT * FROM estoque_consumos WHERE vkt_id='$vkt_id' $fim ORDER BY id DESC LIMIT ".paginacao_limite($registros,$_GET[pagina],$_GET[limitador]));
		echo mysql_error();
		//echo $t;
...
<table cellpadding="0" cellspacing="0" width="100%">
    <tbody dir="dados">
		...
		?>
    	<tr onclick="location='?tela_id=499&pagina=<? if(empty($_GET['pagina'])){echo 1;}else{echo $_GET['pagina'];}?>&limitador=<? if(empty($_GET['pagina'])){echo 30;}else{echo $_GET['limitador'];}?>&fornecedor_id=<?=$fornecedor->id?>&compra_id=<?=$compra->id?>&estoque_id=<?=$almoxarifado->id?>'">
            ...
            <?
            $valor=mysql_fetch_object(mysql_query("SELECT sum( VALOR_INI*QTD_PEDIDA ) as valor FROM `estoque_consumos_item` WHERE pedido_id=".$compra->id))
			?>
