...
function novaVenda(t){
	...
	almoxarifado=$('#almoxarifado_id').val();
	if(cliente!=''&&almoxarifado!=''){
	location.href='?tela_id=201&pagina=<? if(empty($_GET['pagina'])){echo 1;}else{echo $_GET['pagina'];}?>&limitador=<? if(empty($_GET['pagina'])){echo 30;}else{echo $_GET['limitador'];}?>&cliente_id='+cliente+'&almoxarifado_id='+almoxarifado;
...
<form class='form_busca' action="" method="post">
    ...
    <input type="text" id='busca' name="busca" maxlength="44" value=""  autocomplete="off" onkeydown="if(event.keyCode==13){this.parentNode.submit()}" autocomplete="off" value="<?=$_POST['busca']?>"/>
</form>
<div id="some">«</div>
<a href="#" class='s1'>
  	SISTEMA
...
<table cellpadding="0" cellspacing="0" width="100%">
    <tbody dir="dados">
		...
		?>
    	<tr onclick="location='?tela_id=201&pagina=<? if(empty($_GET['pagina'])){echo 1;}else{echo $_GET['pagina'];}?>&limitador=<? if(empty($_GET['pagina'])){echo 30;}else{echo $_GET['limitador'];}?>&cliente_id=<?=$cliente->id?>&venda_id=<?=$venda->venda_id?>'">
            
            
            <td width="60"><?=$venda->venda_id?></td>
          	<td width="250"><?=$venda->razao_social?></td>
