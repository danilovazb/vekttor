<?
	...
	if(empty($select_item)){
		$query=mysql_query($t="INSERT INTO estoque_vendas_item SET vkt_id='$vkt_id', pedido_id='".$venda_id."', produto_id='".$produto_id."', qtd_pedida='1', valor_ini='".$produto->preco_venda."', fatorconversao='{$produto->conversao2}', unidade='$produto->unidade_embalagem'");
		//echo $t;
	...
	$estocado = $saldo->saldo/$produto->conversao2;
	echo  "<tr><td width='150'>".utf8_encode($produto->nome)."<input type='hidden' name='produto_id[]' class='produto_id' value='$produto->id'/></td>
            <td width='60'><div id='class_qtd_origem'><input type='hidden' name='qtd_origem' value='$estocado'></div>".moedaUsaToBr($estocado)." ".substr($produto->unidade_embalagem,0,2)."</td>
            <td width='80'>
           	  <input type='text'  name='qtd[]' class='qtd' value='1,00' onblur='direciona(this)'/>
