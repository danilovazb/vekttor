...
<a href="?tela_id=46" class="navegacao_ativo">
<span></span>   Requisitar Compras
</a>
</div>
<div id="barra_info">
	...
	<form action="" method="get" style="float:left">
		...
		<label for="status">Exibir:</label>
			...
			<option <? if($_GET['status']=="")echo "selected='selected'" ?> value="">Todos</option>
			...
			<option <? if($_GET['status']=="Efetivado")echo "selected='selected'" ?> value="Efetivado">Efetivado</option>
...
<table cellpadding="0" cellspacing="0" width="100%">
	...
	if(strlen($_GET[busca])>0||$_GET['status']){
		$busca_add = "WHERE ";
	}
	if(strlen($_GET[busca])>0&&$_GET['status']!=NULL){
		$busca_add .= "id like '%{$_GET[busca]}%'";
		$busca_add .= "AND status='".$_GET['status']."'";
	}
	if(strlen($_GET[busca])>0&&$_GET['status']==NULL){
		$busca_add .= "id like '%{$_GET[busca]}%'";
	}
	if(strlen($_GET[busca])==0&&$_GET['status']!=NULL){
		$busca_add .= "status='".$_GET['status']."'";
	}
