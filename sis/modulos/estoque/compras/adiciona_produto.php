<?
	...
	if($_POST['acao']=='duplica_compra'){
		...
		$compra_antiga = mysql_fetch_object(mysql_query("SELECT * FROM estoque_compras WHERE id='$compra_antiga_id'"));
		
		//seleciona os itens da compra antiga
		$itens_compra_antiga = mysql_query("SELECT * FROM estoque_compras_item WHERE pedido_id='$compra_antiga_id'");
		
		//adiciona um dia ao atual
		$data_chegada_prevista = mysql_result(mysql_query("SELECT DATE_ADD(CURDATE(), INTERVAL 1 DAY)"),0,0);		
		
		//insere nova compra
		mysql_query($t="INSERT INTO estoque_compras SET
					vkt_id = '$vkt_id',
	...
	//$select_item=mysql_fetch_object(mysql_query($t="SELECT * FROM estoque_compras_item WHERE produto_id='".$produto_id."' AND pedido_id='".$pedido_id."'"));	
	//echo $t."<br>";
	mysql_query($t="INSERT INTO estoque_compras_item SET produto_id='".$produto_id."', pedido_id='".$pedido_id."',fatorconversao=$produto->conversao,fatorconversao2=$produto->conversao2,vkt_id='$vkt_id'");
	...
	if($qtd_estoque<=$produto->estoque_min){
		$color="#B94A48";
	}
	
	//if($select_item->qtd_pedida<=0){
	echo "<tr  id='l".$_POST[nlinhas]."' style='color:$color'>
			<td width='30'>
				$linhas	
		 	</td>
			<td width='80'>
				...
				<span style='display:none;' class='item_id'>$item_id</span>
		 	</td>
		 	<td width='50'>
		 ...
		 <td width='60' align='right' class='cz'><span class='qtd_unidade'>";
            ...
            <td >
				...
				<img src='../fontes/img/menos.png' width='18' height='18' onclick='removeItem(this)' class='remove'/><span style='display:none' id='item$cont' class='item'>$pedido_item->id</span></td>
