...
<table cellpadding="0" cellspacing="0" width="100%">
	...
	$registros= mysql_result(mysql_query("SELECT COUNT(*) FROM rh_empresas re, cliente_fornecedor cf WHERE 
	...
	$q= mysql_query($t="SELECT *, cf.id as cliente_fornecedor_id FROM rh_empresas re, cliente_fornecedor cf WHERE 
		re.cliente_fornecedor_id = cf.id AND
		cf.tipo='Cliente' AND 
		cf.tipo_cadastro='Jurídico' AND 
		re.vkt_id ='$vkt_id' AND 
	...
	while($r=mysql_fetch_object($q)){
			$qtd_func = mysql_num_rows(mysql_query($t=" SELECT * FROM rh_funcionario WHERE 
			empresa_id='$r->id' AND
			status = 'admitidos' AND
			vkt_id='$vkt_id' "));
			
		//quantidade funcionários demitidos
		$qtd_func_demitidos = mysql_num_rows(mysql_query($t=" SELECT * FROM rh_funcionario WHERE 
		empresa_id='$r->id' AND
...
?>
<tr <?=$sel?>onclick="location.href='?tela_id=583&empresa_id=<?=$r->cliente_fornecedor_id?>'">
