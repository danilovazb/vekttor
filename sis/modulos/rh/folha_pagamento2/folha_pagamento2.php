...
$folhas_q=mysql_query($a="SELECT * FROM rh_folha WHERE vkt_id='".$vkt_id."' AND empresa_id='".$empresa_id."' $filtro_ano $filtro_mes ORDER BY id DESC"); echo mysql_error(); 
while($folha=mysql_fetch_object($folhas_q)){
	$total++;
	
	$mes=$mes_extenso[$folha->mes];
	...
	if($total%2){$sel='class="al"';}else{$sel='';}
?>
<tr <?=$sel?> onClick="location.href='?tela_id=586&folha_id=<?=$folha->id?>'">
    <td width="200" ><?=$mes?> <?=$folha->ano?></td>
