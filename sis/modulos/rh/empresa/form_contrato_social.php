	<form onsubmit="return validaForm(this)" class="form_float" method="post" id="form_contrato_social" enctype="multipart/form-data" target="carregador" style="display:none">
	...
	<input name="cliente_fornecedor_id" type="hidden" value="<?=$cliente_fornecedor->cliente_fornecedor_id?>" id="cliente_fornecedor_id2"/>
		<fieldset  id='campos_1' <? if($cliente_fornecedor->tipo_cadastro=="Físico")echo 'style="display:none"'; ?> >
			<legend>
                ...
                <a class="form2"><strong>Contrato Social</strong></a>
			...
			<select name="modelo_id" id="modelo_id">
				...
				<?php
					...
					while($modelo = mysql_fetch_object($modelos)){
						if($modelo->id == $contrato_social->modelo_id){
...
<div id="texto">
 <label style="display:none">
	...
	<img src="modulos/odonto/atendimento/img/edit.png" height="17"/>
                ...
                <input name="salva_formulario_contrato" type="hidden" value="social" />
                 <input name="contrato_id" id="contrato_id_social" type="hidden" value="<?=$contrato_social->id?>" />       
		</fieldset>      
    ...
    <input name="action" type="button" value="Imprimir" id="imprimir_social" style="float:left" />
	<?
	}
	?>
	<input name="action" type="button" id='botao_salvar' onclick="html_to_form('ed','tx_html'); setTimeout('document.getElementById(\'form_contrato_social\').submit();',500)"  value="Salvar" style="float:right"  />
