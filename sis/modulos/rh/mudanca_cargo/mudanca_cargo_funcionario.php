...
<table cellpadding="0" cellspacing="0" width="100%">
	...
	while($r=mysql_fetch_object($q)){
		$cargo_atual = mysql_fetch_object(mysql_query($t="SELECT * FROM rh_alteracao_contrato WHERE vkt_id='$vkt_id' AND funcionario_id='".$r->id."' ORDER BY id DESC LIMIT 1"));
		
		if(empty($cargo_atual)){
			$cargo = $r->cargo;
			
		}else{
			$cargo = $cargo_atual->cargo;
	...
	?>       
    	<tr <?=$sel ?> onclick="window.open('modulos/rh/mudanca_cargo/form.php?id=<?=$r->id?>&empresa1id=<?=$_GET['empresaid']?>','carregador')" >
