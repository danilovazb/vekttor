<?php
	//array em dias
	$tempo_empresa = array(0,45,90,365,730,1095,1825);
?>
...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    <?php
	echo "<tbody>";
	for($i=0;$i<sizeof($tempo_empresa);$i++){
			$tempo1 = $tempo_empresa[$i];
			$tempo2 = $tempo_empresa[$i+1];
			$t = $tempo2/365;
			if($t>=1){
				$exibicao_tempo = $t." Anos";
			}else{
				$exibicao_tempo = $tempo2." Dias";
			}
		
		
			$condicao = "data_admissao > (SELECT DATE_SUB( NOW( ) , INTERVAL $tempo2 DAY)) 
						AND data_admissao <= (SELECT DATE_SUB( NOW( ) , INTERVAL $tempo1 DAY )) ";
			if(empty($tempo2)){
				$condicao = "data_admissao > (SELECT DATE_SUB( NOW() , INTERVAL $tempo1 DAY))";
				$exibicao_tempo = "Acima de 5 anos";
			}
			
			$q = mysql_query($t="
							SELECT COUNT(*) as quantidade_funcionario FROM rh_funcionario WHERE vkt_id='$vkt_id' AND $condicao
							");
		...
		?>       
				<tr <?=$sel?>>
					<td width="70"><?=$exibicao_tempo?></td>
					<td width="80"><?=$r->quantidade_funcionario?></td>
					<td>&nbsp;</td>
	...
	while($empresa = mysql_fetch_object($empresas)){	
		 ...
		 <thead>
		 	<td colspan='3'>$empresa->razao_social</td>
		...
		for($i=0;$i<sizeof($tempo_empresa);$i++){
			...
			$q = mysql_query($t="
							SELECT COUNT(*) as quantidade_funcionario FROM rh_funcionario WHERE vkt_id='$vkt_id' AND empresa_id = '$empresa->id' AND $condicao
							");
		...
		?>       
				<tr <?=$sel?> onClick="location.href='?tela_id=420&empresa1id=<?=$empresa->id?>&tempo1=<?=$tempo1?>&tempo2=<?=$tempo2?>'">
					<td width="70"><?=$exibicao_tempo?></td>
					<td width="80"><?=$r->quantidade_funcionario?></td>
					<td>&nbsp;</td>
