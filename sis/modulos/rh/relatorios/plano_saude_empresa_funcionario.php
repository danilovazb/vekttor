...
<div id="barra_info">
	<input type="button" value="<<" onclick="location.href='?tela_id=569	'"/>
	<?php
		$empresa = mysql_fetch_object(mysql_query("SELECT * FROM cliente_fornecedor WHERE id='".$_GET['empresa_id']."'"));
		$plano_saude = mysql_fetch_object(mysql_query("SELECT * FROM cliente_fornecedor WHERE id='".$_GET['plano_saude_id']."'"));
	?>
	<strong>Emoresa:</strong> <?=$empresa->razao_social?>
...
<table cellpadding="0" cellspacing="0" width="100%">
    <thead>
    	...
    	<tr>
            ...
            <td width="100">Valor Funcionário</td>            
...
<table cellpadding="0" cellspacing="0" width="100%">
	...
	if(strlen($_GET[ordem])>0){
		...
		$ordem =  'nome';
			...
			$q = mysql_query($t="
					  ...
					  WHERE 
					  	f.vkt_id        = '$vkt_id' AND
						f.empresa_id     = '".$_GET['empresa_id']."' AND						
						f.plano_saude_id = '".$_GET['plano_saude_id']."' AND
						f.status    != 'demitidos'
		...
		$dependentes_plano = mysql_fetch_object(mysql_query($t=
			"SELECT 
							COUNT(*) as qtd_dependente
						FROM 
							rh_funcionario_dependentes
							
						WHERE
							funcionario_id=$r->id AND
							plano_saude='sim'
						"
		));
		echo mysql_error();
		...
		//while($dependente_plano=mysql_fetch_object($dependentes_plano)){
			$total_dependente = $r->plano_saude_valor_depenente*$dependentes_plano->qtd_dependente;
	...
	?>       
    	<tr <?=$sel ?>>
            ...
            <td width="100"><?=$dependentes_plano->qtd_dependente?></td>
...
<?php
		...
		$total_valor_funcionario += $r->plano_saude_valor_funcionario;
		$total_qtd_dependente    += $dependentes_plano->qtd_dependente;
		$total_valor_dependente  += $total_dependente;
		$total_valor_plano       += $total_plano;
		$total_valor_descontado  += $r->plano_saude_valor_descontado;
		}
?>
		</tbody>
