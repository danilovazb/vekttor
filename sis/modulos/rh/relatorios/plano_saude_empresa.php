...
<table cellpadding="0" cellspacing="0" width="100%">
    <thead>
    	...
    	<tr>
            ...
            <td width="100">Qtd Funcionários</td>            
...
<table cellpadding="0" cellspacing="0" width="100%">
	...
	$empresas= mysql_query($t="SELECT *, cf.id as cliente_forencedor_id FROM 
		...
		while($empresa=mysql_fetch_object($empresas)){
			$total_qtd_funcionario_empresa = 0;
			$vlr_funcionario_empresa = 0;
        	$qtd_dependentes_empresa = 0;
			$vlr_dependentes_empresa = 0;
			$total_plano_empresa     = 0;
			$total_plano             = 0;
			$total_desconto_empresa  = 0;		
			
			$q = mysql_query($t="
					  SELECT 
					  	plano_saude_id,
					  	COUNT(*) as qtd_funcionario,
					  	SUM(plano_saude_valor_funcionario) as total_funcionario,
						SUM(plano_saude_valor_descontado) as valor_descontado, 
						(SELECT razao_social FROM cliente_fornecedor WHERE id=f.plano_saude_id) as plano_saude
					  ...
					  WHERE 
						...
						f.empresa_id = '$empresa->cliente_forencedor_id' AND						
						f.status    != 'demitidos' 
						$filtro
						GROUP BY(plano_saude_id)
		...
		?>
        		<thead>
        		<tr <?=$sel ?> >
      				<td colspan="8"><?=$empresa->razao_social?></td>
            		
       			</tr>
            </thead>
        <?php				
		while($r=mysql_fetch_object($q)){
		$dependentes_plano = mysql_query($t=
			"SELECT 
						...
						WHERE
							...
							rh_funcionario.plano_saude_id = $r->plano_saude_id AND
	...
	?>       
    	<tr <?=$sel ?> onclick="location.href='?tela_id=576&empresa_id=<?=$empresa->id?>&plano_saude_id=<?=$r->plano_saude_id?>'">
      		<td width="250"><?=$r->plano_saude?></td>
            <td width="100"><?=$r->qtd_funcionario?></td>            
            <td width="100"><?=MoedaUsaToBr($r->total_funcionario)?></td>
            <td width="100"><?=$qtd_dependente?></td>
            <td width="100" ><?=MoedaUsaToBr($total_dependente)?></td>
          	<td width="100" ><?=MoedaUsaToBr($total_plano)?></td>
            <td width="100"><?=MoedaUsaToBr($r->valor_descontado)?></td>
          	<td width="" class="wp"></td>
        </tr>
      
<?
		$total_qtd_funcionario_empresa += $r->qtd_funcionario;
		$vlr_funcionario_empresa += $r->total_funcionario;
        $qtd_dependentes_empresa += $qtd_dependente;
		$vlr_dependentes_empresa += $total_dependente;
		$total_plano_empresa     += $total_plano;
		$total_desconto_empresa  += $r->valor_descontado;	
		
		$total_qtd_funcionario   += $r->qtd_funcionario;
		$total_valor_funcionario += $r->total_funcionario;
		$total_qtd_dependente    += $qtd_dependente;
		$total_valor_dependente  += $total_dependente;
		$total_valor_plano       += $total_plano;
		$total_valor_descontado  += $r->valor_descontado;
		}
?>
		</tbody>
        <thead>
        <tr <?=$sel ?>>
      		<td width="250"></td>
            <td width="100"><?=$total_qtd_funcionario_empresa?></td>            
            <td width="100"><?=MoedaUsaToBr($vlr_funcionario_empresa)?></td>
            <td width="100"><?=$qtd_dependentes_empresa?></td>
            <td width="100" ><?=MoedaUsaToBr($vlr_dependentes_empresa)?></td>
          	<td width="100" ><?=MoedaUsaToBr($total_plano_empresa)?></td>
            <td width="100"><?=MoedaUsaToBr($total_desconto_empresa)?></td>
          	<td width="" class="wp"></td>
        </tr>
        </thead>
<?php
