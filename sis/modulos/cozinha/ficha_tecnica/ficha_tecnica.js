...
function addProduto(t){
	if(t.getAttribute('src')=='../fontes/img/mais.png'){
		...
		$(".produto_item:odd").addClass('al');
		$(".produto_item:even").removeClass('al');
		
	}
}


function produto_add_fica(t){
		...
		/***** atribui a coloração *****/
		nova_class = ($(t).parent().parent().hasClass('al'))?'':'al';
...
function calculaTotal(t){
	...
	if(tr.getElementsByTagName('input')[1].value==''){
		...
		$("#total_qtd").text(moedaUsaToBR(soma_qtd.toFixed(2)));
		//add
		$("#peso").val(moedaUsaToBR(soma_qtd.toFixed(2)));
...
function calculaPerCapta(total, percapta, destino){
	...
	if(peso_total!=null&&peso_total>0){
		if(peso_percapta!=null&&peso_percapta>0){
			...
			document.getElementById(destino).innerHTML=moedaUsaToBR(resultado.toFixed(3));
			$("#"+destino+"_pessoas").val(moedaUsaToBR(resultado.toFixed(3)));
		}
	}
	
}

function calculaPerCaptaPessoas(total, percapta, destino){
	peso_total = moedaBrToUsa(document.getElementById(total).value);
	peso_percapta = moedaBrToUsa(percapta.value);
	resultado= (peso_total/peso_percapta);
	...
	if(peso_total!=null&&peso_total>0){
		if(peso_percapta!=null&&peso_percapta>0){
			resultado= (peso_total/peso_percapta);
			document.getElementById(destino).innerHTML=moedaUsaToBR(resultado.toFixed(3));
			$("#"+destino).val(moedaUsaToBR(resultado.toFixed(3)));
			$("#span_"+destino).html(moedaUsaToBR(resultado.toFixed(3)));
		}
	}
...
function adicionaGrupo(t){
		if($(t).attr('src')=='../fontes/img/mais.png'){
			...
			//adiciona o campo para o usuário digitar o nome do grupo
			...
			//$(t).parent().parent().find('.adiciona_grupo').live('keyup',function(e){
			input_grupo.bind('keyup',function(e){
				if(e.which==13){
					if($(this).val()!='' ){
						
					//copia a ultima linha
					var ultima_linha = $(t).parent().parent().prev(".produto_item");
					cor= (ultima_linha.hasClass('al'))?'':'al';
					var nova_linha = $("<tr class='produto_item "+cor+"'>"+ultima_linha.html()+"</tr>");
