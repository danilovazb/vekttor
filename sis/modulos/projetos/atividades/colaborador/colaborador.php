...
#tarefas_concluidas{ display:none;}
...
?>
	...
	<div class="<?=$bga?>" onclick="location='?tela_id=96&projeto_id=todas'">
    	<div id='' class="infd" title="Dias para conclusao"><?=@mysql_result(mysql_query("SELECT count(*) FROM projetos_atividades WHERE (funcionario_id='$usuario_id' OR usuario_id_cadastrou='$login_id') AND situacao in ('0','2','3')"),0,0);
?></div>
...
<?php 
	...
	p.nome,p.id,count(a.id) as quantidade, 
	DATEDIFF(p.data_limite,date(now())) as limite 
	FROM projetos_atividades as a, projetos as p 
	WHERE a.projeto_id=p.id  AND 
	(a.funcionario_id = '$login_id' OR a.usuario_id_cadastrou='$login_id')

	AND a.situacao in ('0','2','3') GROUP BY a.projeto_id  ORDER BY p.data_limite,p.nome  ");
//	echo $t;
		while($p=mysql_fetch_object($sql)){
			$conta_projeto++;
			$projeto_nome[$p->id]= $p->nome;
  ...
  <tbody>
	...
	$q = mysql_query($t="
			...
			AND
				situacao <>'1'
			AND 
				(funcionario_id = '$login_id' OR usuario_id_cadastrou='$login_id')
				$filtro_projeto 
				$filtro_tipo_atividade 
				$filtro_tipo_situacao  
			ORDER BY 
				projeto_id,ordenacao_funcionario ASC ");
/*
atrasadas
excedido
pendentedeaprovacao
...
//			echo $t;	
	...
	?><tr <?=$sel?>  ri="<?=$r->id?>" title="<?=$atividade->nome."$falta_ou_passaram por $usuario_cadastrou->nome"?>" >
      <td  class='atv ' title='<?=$projeto_nome[$r->projeto_id]?>'><?
	 if($r->prioridade==1){
      ?><div class='star'></div><?
	}
	...
	?></div><input type="checkbox" style="visibility:<?=$visibility?>" /> 
	  ...
	  ?><div style=" height:8px;border:1px solid #666; background:#FFF; "><div id='p<?=$r->id?>' style="height:8px;width:<?=$percentual_concx?>%; background:<?=$colorp?>"></div>
      </div></td>
	...
	?></div><input type="checkbox" style="visibility:<?=$visibility?>" checked="checked" /> 
      ...
      <td ><?
		 $percentual_concx=100;
			$colorp='#009900';
	  ?>
        <div style=" height:8px;border:1px solid #666; background:#FFF; ">
...
<div style="height:120px; overflow:auto;" id='tarefas_concluidas'>
