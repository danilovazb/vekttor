...
<table cellpadding="0" cellspacing="0" width="100%" id="tabela_dados" >
	...
	<?php 
		if(!empty($_GET['de'])&&!empty($_GET['ate'])){
			...
			$mes_atual=date("m");
			$filtro = "AND MONTH(data_hora_interacao) = '$mes_atual'";
		...
		*/
		...
		while($r=@mysql_fetch_object($sql)){
			...
			$resultado_vendedor[$c]["qtd_telefonemas"] = @mysql_num_rows(mysql_query($t="SELECT * FROM revenda_contato_interacao WHERE vendedor_id=$r->vendedor_id AND o_que_gerou='1' $filtro $busca"));
			//echo $t;
			$soma_num_telefonemas+=$resultado_vendedor[$c]["qtd_telefonemas"];
			
			$resultado_vendedor[$c]["qtd_reunioes"] = @mysql_num_rows(mysql_query("SELECT * FROM revenda_contato_interacao WHERE vendedor_id=$r->vendedor_id AND o_que_gerou='3' $filtro $busca"));
			$soma_num_reunioes+=$resultado_vendedor[$c]["qtd_reunioes"];
			
				$resultado_vendedor[$c]["qtd_visitas"] = @mysql_num_rows(mysql_query("SELECT * FROM revenda_contato_interacao WHERE vendedor_id=$r->vendedor_id AND o_que_gerou='2' $filtro $busca"));
			$soma_num_visitas+=$resultado_vendedor[$c]["qtd_visitas"];
...
?>      
    	<tr <?=$sel?> id="<?=$r->id?>" onclick="location.href='?tela_id=360&vendedor_id=<?=$resultado_vendedor[$c]["id"]?>&data_ini=<?=$data_ini?>&data_fim=<?=$data_fim?>'">
          <td width="50"><?=$resultado_vendedor[$c]["id"]?></td>
          <td width="200"><?=$resultado_vendedor[$c]["nome"]?></td>
          <td width="100"><?php if(!$resultado_vendedor[$c]["num_contatos"]>0){ echo "0";}else{ echo $resultado_vendedor[$c]["num_contatos"];}?></td>
          <td width="100"><? if(!$resultado_vendedor[$c]["qtd_telefonemas"]>0){ echo "0";}else{ echo $resultado_vendedor[$c]["qtd_telefonemas"];}?></td>
          <td width="100"><? if(!$resultado_vendedor[$c]["qtd_reunioes"]>0){ echo "0";}else{ echo $resultado_vendedor[$c]["qtd_reunioes"];}?></td>
          <td width="100"><? if(!$resultado_vendedor[$c]["qtd_visitas"]>0){ echo "0";}else{ echo $resultado_vendedor[$c]["qtd_visitas"];}?></td>          
          <td></td>
        </tr>
...
<table cellpadding="0" cellspacing="0" width="100%" style="border-top:solid thin black">
    <thead>
          ...
          <td width="100"><?php echo $soma_num_visitas?></td>
          <td></td>
        </tr>
    </thead>
</table>
