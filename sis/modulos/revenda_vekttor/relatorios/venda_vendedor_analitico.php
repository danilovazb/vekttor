...
<table cellpadding="0" cellspacing="0" width="100%" id="tabela_dados" >
	...
	<?php 
		...
		//$vendedor = mysql_fetch_object(mysql_query("SELECT * FROM rh_funcionario WHERE id='".$_GET['vendedor_id']."'"));
		$registros= @mysql_result(mysql_query($t="SELECT COUNT(*) FROM vekttor_venda
							WHERE vendedor_id = ".$_GET['vendedor_id']."
							$filtro
							$busca"),0,0);
							//echo $t;
		$sql = @mysql_query($t="SELECT * FROM vekttor_venda
							WHERE vendedor_id = ".$_GET['vendedor_id']."
							$filtro
							$busca
		...
		$resultado_vendedor = array();
			while($r=@mysql_fetch_object($sql)){
			 ...
			 $valores = @mysql_fetch_object(mysql_query($t="SELECT valor_implantacao,valor_mensalidade FROM vekttor_venda WHERE id=$r->id"));
			 $cliente_vekktor = @mysql_fetch_object(mysql_query($t="SELECT * FROM clientes_vekttor WHERE id=$r->cliente_vekttor_id"));
		?>      
    	<tr <?=$sel?> id="<?=$r->id?>">
          <td width="200"><?=$cliente_vekktor->nome?></td>
          <td width="100"><?=MoedaUsaToBr($valores->valor_implantacao)?></td>
          <td width="100"><?=MoedaUsaToBr($valores->valor_mensalidade)?></td>
