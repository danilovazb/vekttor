...
<div id="barra_info">
	<input type="button" value="<<" style="margin-top:3px;" onclick="location.href='?tela_id=376'" />
...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    $registros= mysql_result(mysql_query("SELECT count(*) FROM emailmarketing_envio WHERE vkt_id = '$vkt_id' AND
		emailmarketing_id = '$id' AND
		sucesso='0'
	...
	$q= mysql_query($t="SELECT * FROM 
		emailmarketing_envio
		WHERE
		vkt_id = '$vkt_id' AND
		emailmarketing_id = '$id' AND
		sucesso='0'
		$busca_add $filtro ORDER BY data ".$_GET['ordem_tipo']." DESC LIMIT ".paginacao_limite($registros,$_GET[pagina],$_GET[limitador]));
	//echo $t;
	while($r=mysql_fetch_object($q)){
		
		$cliente_fornecedor = mysql_fetch_object(mysql_query("SELECT * FROM cliente_fornecedor WHERE id='$r->cliente_fornecedor_id'"));
	...
	?>
<tr <?=$sel?>>
<td width="230"><?=$cliente_fornecedor->nome?></td>
<td width="230"><?=$cliente_fornecedor->email?></td>
