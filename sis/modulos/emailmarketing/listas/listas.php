...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    $registros= mysql_result(mysql_query("SELECT count(*) FROM emailmarketing_listas WHERE vkt_id = '$vkt_id' $busca_add"),0,0);
    
	
	// colocar a funcao da paginação no limite
	$q= mysql_query($t="SELECT * FROM 
		emailmarketing_listas
		WHERE
		vkt_id = '$vkt_id'
