...
<div id="barra_info">
    <select onchange="location='?tela_id=170&situacao='+this.value" name="situacao">
		<?
...
<table cellpadding="0" cellspacing="0" width="100%">
	...
	if(strlen($_GET[busca])>0){
		$busca_add = "AND d.identificacao like '%{$_GET[busca]}%'";
	}
	
	if($_GET['situacao']>0){
		$filtro_situacao="AND c.situacao='".($_GET['situacao']-1)."'";
	}
	else{ $filtro_situacao="";}
	
	if($cliente_tipo_id!='7'){
		$filtro_corretores=" AND c.corretor_id='$login_id' ";
	}else{$filtro_corretores=' AND 1=1 ';}
	
	$registros= mysql_result(mysql_query("
	SELECT
		count(*) 
	FROM 
		contrato as c, disponibilidade as d
	WHERE 
		c.disponibilidade_id = d.id
		$filtro_corretores
		AND c.situacao='2' $busca_add ORDER BY d.identificacao"),0,0);
	...
	WHERE
		...
		c.cliente_fornecedor_id = cf.id AND
		c.situacao='2'
	...
	while($r=mysql_fetch_object($q)){
		...
		if($r->sit==2)$situacao="Aprovado";
		if($r->sit==3)$situacao="Pago";
	?>
	
    	<tr onclick="location='?tela_id=171&id=<?=$r->id;?>'">
