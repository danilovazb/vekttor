...
<div id="barra_info"><strong>Cliente:</strong> <a href="modulos/administrativo/clientes/form.php?id=<?=$cliente_fornecedor->id ?>" target="carregador" title="Dados do Cliente"><?=$cliente_fornecedor->razao_social ?></a> <strong>Valor:</strong> R$ <?=moedaUsaToBr($contrato->valor)?> 
...
<input name="action" type="button"  value="Tranferencia de Titularidade" />
  ...
  <input type="button" value="Contrato" <?php if($contrato->situacao == '0'){ echo "disabled='disabled'";}?>style="float:right; margin:3px 3px 0 0 ;" onclick="window.open('modulos/administrativo/vendas/contrato.php?id=<?=$_GET[id]?>','_BLANK')" />
</div>
</div>
