...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    $registros= mysql_result(mysql_query("SELECT count(*) FROM contrato JOIN disponibilidade ON contrato.disponibilidade_id=disponibilidade.id $situacao AND usuario_id='".$_SESSION['usuario']->id."' $busca_add ORDER BY identificacao"),0,0);
	...
	$q= mysql_query("SELECT *,contrato.situacao as sit, contrato.id as idc 
						FROM 
							contrato 
						JOIN disponibilidade 
						ON contrato.disponibilidade_id=disponibilidade.id $situacao AND usuario_id='".$_SESSION['usuario']->id."' $busca_add ORDER BY ".$ordem." ".$_GET['ordem_tipo']." LIMIT ".paginacao_limite($registros,$_GET[pagina],$_GET[limitador]));
	...
	?>
    	...
    	<tr onclick="location='?tela_id=166&id=<?=$r->idc?>'">
