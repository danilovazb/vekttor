...
<script>
	...
	$("#filtrar").live("click",function(){
		var grupo_id = $("#grupo_id").val();
		location.href='?tela_id=16&grupo_id='+grupo_id+'&filtro=filtrar';
	});
	...
	$("#exportar").live("click",function(event){
			...
			window.open('modulos/administrativo/clientes/form_exportar.php?tipo=Fornecedor','carregador');	
	});
	
	$("#aniversariante").live("click",function(event){
			//alert('ater');
			location.href='?tela_id=16&aniversariantes=sim';	
	});
	...
	$('.remover_foto').live('click',function(){
		...
		$("#form_fornecedor").attr("target","carregador");
		$("#form_fornecedor").submit();
		$("#form_fornecedor").attr("target","");
		$('#action2').val("");
	});
	...
	$('.download_documento').live('click',function(){
		...
		window.open("modulos/administrativo/fornecedores/downloadDocumento.php?documento_id="+id,"carregador");
	});	
</script>
<div id='conteudo'>
<div id='navegacao'>
...
<a href="?tela_id=16" class="navegacao_ativo">
<span></span>    Fornecedores 
</a>
</div>
<div id="barra_info">
	 <select name="grupo_id" id="grupo_id" style="float:left; margin-top:3px;" title="Exportar o cliente" data-placement="right">
    	<option value="">Grupos</option>
        <option value="novo">Adicionar</option>
		<?php
			$grupos = mysql_query("SELECT * FROM cliente_fornecedor_grupo WHERE tipo = 'F' AND vkt_id='$vkt_id'");
...
<table cellpadding="0" cellspacing="0" width="100%">
	...
	if(strlen($_GET[busca])>0){
		...
		$busca_add = "AND (nome_fantasia like '%{$_GET[busca]}%' OR cnpj_cpf LIKE '%{$_GET[busca]}%')";
	}
	...
	// necessario para paginacao
    $registros= mysql_result(mysql_query("SELECT count(*) FROM cliente_fornecedor WHERE tipo='Fornecedor' AND cliente_vekttor_id ='$vkt_id' $busca_add ORDER BY nome_fantasia"),0,0);
	...
	$q= mysql_query("SELECT * FROM cliente_fornecedor WHERE tipo='Fornecedor' AND cliente_vekttor_id ='$vkt_id' $busca_add $filtro ORDER BY ".$ordem." ".$_GET['ordem_tipo']." LIMIT ".paginacao_limite($registros,$_GET[pagina],$_GET[limitador]));
	...
	?>
    	...
    	<tr <?=$sel?>onclick="window.open('<?=$caminho?>form.php?id=<?=$r->id?>','carregador')">
			...
			<td width="150"><?=$r->ramo_atividade ?></td>
            <td><?
