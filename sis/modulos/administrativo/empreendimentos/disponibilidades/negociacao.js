...
function campos_ato_comissao(qtd,ato_valor_parcela,comissao_valor_parcela,total_valor_parcela){
	$('#campos_ato').html('');
	...
	$("#ato_restante").val(moedaUsaToBR((total_ato-total_comissao).toFixed(2)));
...
function calcula_outras_parcelas(){
}
...
function selectNegociacao(t){
	if(t.value>0){
		...
		campos_ato_comissao(ato_parcelas,moedaUsaToBR(ato_valor_parcela),moedaUsaToBR(comissao_valor_parcela),moedaUsaToBR(parseFloat(comissao_valor_parcela)+parseFloat(ato_valor_parcela)));
