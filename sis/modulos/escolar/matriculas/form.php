         ...
         <label id="cnpj" style="width:120px; margin-right:22px;<?=$desaparece['Físico']?>">
			...
			<label style="width:240px; margin-right:23px;">
             ...
             <label style="width:70px;">
				Nascimento
				<input type="text" mascara='__/__/____' id='f_nascimento' name="f_nascimento" value="<?=dataUsaToBr($responsavel->nascimento)?>" retorno="focus|Digite a data de nascimento" valida_minlength='1' valida_data='01/01/0001,99/99/9999'/>
			</label>
...
<div style="clear:both"></div>			<label style="width:120px;">
  ...
  <div id='pagina_id_2' class='paginamatricula axluml' style="width:770px;  display:none">
            ...
            <div style="float:left; width:620px;">
           	...
           	<label><input type="checkbox" name="mesmo" value="0" style="width:inherit" class="aluno_mesmo" />Este Aluno tem os mesmos dados do responsável</label>
            <div style="clear:both;"></div>
           	<label><input type="checkbox" name="mesmo" value="0" style="width:inherit" class="endereco_mesmo" />Este Aluno reside no mesmo local que o responsavel</label>
            <div style="clear:both;"></div>
            </div>
           ...
           <!-- <label style="width:250px;">Turma
            ...
            <? 
			if($matricula->id>0){
				$pago[$matricula->pago]="selected='selected'"; 
			}else{
				$pago['N']="selected='selected'";
			}
			if($vkt_id=='8'){
				$display='';
			}else{
			...
			?>
            <label <?=$display?>>Situação
            	<select name="pago">
                	<option <?=$pago['S']?> value="S">Pago</option>
                    <option <?=$pago['N']?> value="N">Não Pago</option>
                    <option <?=$pago['C']?> value="C">Cancelado</option>
                </select>
            </label>
...
$q = mq($t="SELECT e.* FROM escolar_horarios as h, escolar_escolas as e WHERE  h.vkt_id='$vkt_id' AND h.escola_id=e.id AND h.periodo_id = '$matricula->periodo_id' group by e.id");
while($r=mf($q)){
	if($r->id==$matricula->escola_id){$s='selected="selected"';}else{$s='';}
    echo "<option value='$r->id' $s>$r->nome</option>";
}
...
$q = mq($t="SELECT c.* FROM escolar_horarios as h, escolar_cursos as c WHERE  h.vkt_id='$vkt_id' AND h.curso_id=c.id AND h.periodo_id = '$matricula->periodo_id' AND h.escola_id='$matricula->escola_id' group by c.id");
while($r=mf($q)){
 	if($r->id==$matricula->curso_id){$s='selected="selected"';}else{$s='';}
   echo "<option value='$r->id' $s>$r->nome</option>";
}
...
$q = mq($t="SELECT m.* FROM escolar_horarios as h, escolar_modulos as m WHERE  h.vkt_id='$vkt_id' AND h.modulo_id=m.id AND  h.periodo_id = '$matricula->periodo_id' AND h.escola_id='$matricula->escola_id' AND h.curso_id='$matricula->curso_id' group by m.id");
//echo $t;
while($r=mf($q)){
 	if($r->id==$matricula->modulo_id){$s='selected="selected"';}else{$s='';}
    echo "<option value='$r->id' $s>$r->nome</option>";
}
...
$q = mq($t="SELECT h.* FROM escolar_horarios as h WHERE  h.vkt_id='$vkt_id' AND h.periodo_id = '$matricula->periodo_id' AND h.escola_id='$matricula->escola_id' AND h.curso_id='$matricula->curso_id' AND h.modulo_id ='$matricula->modulo_id' ");

while($r=mf($q)){
 	if($r->id==$matricula->horario_id){$s='selected="selected"';}else{$s='';}
    if(strlen($r->nome)>0){
		 $nome = $r->nome; 
	}else{
	...
	echo "<option value='$r->id' valor='$r->valor' valor_bolsista='$r->valor_bolsista' $s>$nome</option>";
}
...
$q = mq($t="SELECT * FROM escolar_salas WHERE vkt_id = '$vkt_id'  AND horario_id='$matricula->horario_id'");
while($r=mf($q)){
 	if($r->id==$matricula->sala_id){$s='selected="selected"';}else{$s='';}
    echo "<option value='$r->id' $s>$r->nome</option>";
}
...
?>
    ...
    <option value="REMATRÍCULA" <?=$tpmat['REMATRÍCULA']?>>REMATRÍCULA</option>
            ...
            <label class="lb_valor">
            Valor Matricula
            <div> R$ <span><?=number_format($matricula->valor,2,',','.')?></span></div>
            <input type="hidden" name="valor[]" class="valor" value="<?=$matricula->valor?>"  valida_minlength="1" retorno='submit|Selecione um Horário para ser definido o valor' />
			</label>
            <label class="lb_data_pagamento" style="width:120px">
            Data de Vencimento
            <input type="text" id='data_vencimento[]' mascara='__/__/____' name="data_vencimento[]" calendario='1' style="width:80px" value="<?=dataUsaToBr($matricula->data_vencimento ); ?>" valida_data='01/01/0001,99/99/9999' />
			</label> 
            <?
            if($matricula->id>0){
			?>
            <label  style="width:150px">
            Situação Pagamento <br />
            <?
            if($matricula->pago=='S'){
				echo "<strong>Pago - $matricula->arquivo_retorno_id</strong>";	
			}elseif($matricula->pago=='C'){
				echo "<strong>Cancelado - $matricula->arquivo_retorno_id</strong>";	
			}else{
				echo "<strong>Pendente - $matricula->arquivo_retorno_id</strong>";	
			}
