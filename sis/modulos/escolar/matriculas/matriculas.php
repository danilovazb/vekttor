...
while($periodo=mysql_fetch_object($s_periodo)){
  ...
  while($escola=mysql_fetch_object($s_escola)){
	...
	while($curso=mysql_fetch_object($s_cursos)){
      ...
      <button type="button" id="val-matricula" title="Altarar o valor da Matr&iacute;cula do Aluno" data-placement='right'>Valor Matr&iacute;cula</button>
        ...
        <table cellpadding="0" cellspacing="0" width="100%" id="lista">
		  ...
		  */
			...
			if( strlen( $_GET[busca]) > 0 ){
				...
				$busca_add = " 
				AND 
				(		a.nome LIKE '%{$_GET[busca]}%'
					OR
						c.razao_social LIKE '%{$_GET[busca]}%'
					OR
						a.cpf = '{$_GET[busca]}'
					OR
						((a.cpf ='' OR a.cpf ='0') AND c.cnpj_cpf = '{$_GET[busca]}')
					OR 
						m.id = '{$_GET[busca]}'
				)
			...
			COUNT(*)
FROM
	escolar_matriculas m, escolar_alunos as a, cliente_fornecedor as c
WHERE
	m.vkt_id='$vkt_id'	 
AND
	m.aluno_id = a.id 
AND 
	m.responsavel_id = c.id
$busca_pago
$busca_add
$add2
...
//\\ echo "<PRE>$t</PRE>";
 ...
 from escolar_alunos as a WHERE a.id='$r->aluno_id'"));
           ...
           ?><tr <?php echo $cl; ?> id='<?=$r->matricula_id?>'>
            ...
            <td onclick="l(<?=$r->aluno_id?>)"><?php echo $aluno->nome; ?></td>
            <td><?php echo $r->aluno_cpf; ?></td>
            <td><?php echo $r->responsavel_cpf; ?></td>
            <td><?php echo $aluno->age ?></td>
            <td><?=$escola->nome." - ".$curso->nome." - ".$modulo->nome." - $sala->nome - ".$horario->nome ?> <?=$valor?></td>
            </tr>
<?php
            }
            ?>	
