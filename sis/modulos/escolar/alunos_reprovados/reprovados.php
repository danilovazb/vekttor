    ...
    <thead>
    	<tr>
           <td width="50">Codigo</td>
            <td width="75">Totvs</td>
            <td width="190"><?=linkOrdem("Nome","nome_fantasia",1)?></td>
          	<td width="90"><?=linkOrdem("Telefone","Telefone",0)?></td>
          	<td width="140"><?=linkOrdem("Email","Email",0)?></td>
          	<td width="100"><?=linkOrdem("Turma","Turma",0)?></td>
            <td width="100"><?=linkOrdem("Turno","Turno",0)?></td>
			<td></td>
        </tr>
    </thead>
</table>
...
<table cellpadding="0" cellspacing="0" width="100%">
   ...
   //echo $t;
	...
	$q= mysql_query($t="SELECT a.*,reprovado.* FROM escolar_alunos a, escolar_alunos_reprovado reprovado 
	WHERE a.id=reprovado.aluno_id AND reprovado.vkt_id='$vkt_id' $busca_add ORDER BY ".$ordem." ".$_GET['ordem_tipo']." LIMIT ".paginacao_limite($registros,$_GET[pagina],$_GET[limitador]));
	//echo $t;
