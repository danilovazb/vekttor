...
<div id="barra_info">
	<input type="button" value="<<" onclick="location.href='?tela_id=297&&professor=<?=$_GET['professor']?>'">	
</div>
...
<table cellpadding="0" cellspacing="0" width="100%" id="tabela_dados" style="margin:0px;">
   ...
   //echo $t;
	...
	while($r=mysql_fetch_object($q)){
		...
		if(!empty($r->extensao)){
			echo "<a href='modulos/escolar/area_aluno/aulas/arquivos/$r->id$r->extensao' style='float:right;color:white' target='_blank'>Baixar Arquivo</a>";
		}
		echo "
				</td>
            </tr>";
		...
		if($r->aluno_id==$r->de){
		 echo "<td style='border:1px solid #CCC;'>
		 		...
		 		<strong>$aluno->nome:</strong> ".$r->mensagem."
			...
			echo "<td style='border:1px solid #CCC;'><strong>Voc&ecirc;: </strong>".$r->mensagem."
