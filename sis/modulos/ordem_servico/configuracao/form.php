...
include("_ctrl.php"); 
$sql=mysql_fetch_object(mysql_query("SELECT * FROM os_configuracao WHERE id='$vkt_id'"));
$sqlConta = mysql_fetch_object(mysql_query(" SELECT * FROM os_conta WHERE vkt_id = '$vkt_id'"));
	...
	if(empty($sql->img_rodape)){
		...
		$img_r="modulos/ordem_servico/configuracao/img/".$vkt_id."_r.".$sql->img_cabecalho;
	}
?>
<style>
...
<div>
	...
	<fieldset>
        ...
        <? if(!empty($sql->img_cabecalho)){ ?>
        <img id="img_cabecalho" src="<?="modulos/ordem_servico/configuracao/img/".$sql->id."_c.".$sql->img_cabecalho?>" height="90" width="100%"/>
        <a style="display:block;" href="#" onclick="document.getElementById('img_cabecalho').setAttribute('src',''); document.getElementById('img_cabecalho').style.display='none';this.style.display='none';window.open('modulos/ordem_servico/configuracao/deletarfoto.php?img=cabecalho&extensao=<?=$sql->img_cabecalho?>&id=<?=$obj->id?>','carregador')">Remover Foto</a>
        <? } ?>
        <div style="clear:both"></div>
        <label style="width:200px;">
		...
		<input type="file" name="img2" id="img2"/>
         ...
         <? if(!empty($sql->img_rodape)){ ?>
        <img id="img_rodape" src="<?="modulos/ordem_servico/configuracao/img/".$sql->id."_r.".$sql->img_rodape?>" height="90" width="100%"/>
        <a style="display:block;" href="#" onclick="document.getElementById('img_rodape').setAttribute('src','');document.getElementById('img_rodape').style.display='none';this.style.display='none'; window.open('modulos/ordem_servico/configuracao/deletarfoto.php?img=rodape&extensao=<?=$sql->img_rodape?>&id=<?=$obj->id?>','carregador')">Remover Foto</a>
        <? } ?>
        <div style="clear:both"></div>
        ...
        <label>Valor total para desconto na O.S<br/>
        	<input type="text" name="total_desconto" id="total_desconto" style="width:100px;" decimal='2' sonumero='1' value="<?=moedaUsaToBr($sql->valor_total_desconto)?>">
        </label>
</fieldset>
<!-- ABA CONFIG CONTA -->
