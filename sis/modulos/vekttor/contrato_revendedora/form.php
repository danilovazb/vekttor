...
<div>
    ...
    <span>Contrato Revendedora</span></div>
    </div>
	...
	<fieldset  id='campos_1' >
		...
		<?php
			$contrato = mysql_fetch_object(mysql_query($t="SELECT * FROM contrato_modelo_revendedora WHERE id='$vkt_id'"));
...
<div style="clear:both"></div>
       ...
       <iframe id='ed' name='ed' width="75%" style="height:345px; background:#FFF;  overflow:scroll;float:left" onload="this.contentWindow.document.designMode='on'; this.contentWindow.document.body.innerHTML=document.getElementById('tx_html').value;" frameborder="0"class="edtx"></iframe>
        ...
        <div id="esquerda" style="float:right;overflow:auto">
            ...
            <div style="clear:both"></div>
            <a href="#" onclick="ti('InsertHTML',this.innerHTML) "><strong>@contratante_porc_mensalidade</strong></a>
            <div style="clear:both"></div>
            <a href="#" onclick="ti('InsertHTML',this.innerHTML) "><strong>@contratante_porc_implantacao</strong></a>
        </div>
	</fieldset>
	<input name="id" type="hidden" value="<?=$vkt_id?>" />
	<input name="salva_formulario_contrato_revendedora" type="hidden" value="1" />
