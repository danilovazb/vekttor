...
<thead>
    	<tr>
          ...
          <td width="70">Qtd Implant</td>
          <td width="65">Vlr Implant</td>
          <td width="65">% Implant</td>
          <td width="65">Qtd Mensal</td>
...
<table cellpadding="0" cellspacing="0" width="100%" id="tabela_dados" >
	...
	<?php 
		...
		$valores_vendas = mysql_fetch_object(mysql_query("SELECT SUM(valor_implantacao) as vi,SUM(valor_mensalidade) as vm FROM vekttor_venda $filtro"));
		
		while($r=@mysql_fetch_object($sql)){
			$qtd = @mysql_num_rows(mysql_query($t="SELECT * FROM vekttor_venda $filtro AND revenda_franquia_id = $r->vkt_id"));
			$soma_qtd += $qtd;
			//echo $t;
			$valores = @mysql_num_rows(mysql_query("SELECT SUM(valor_implantacao) as vi,SUM(valor_mensalidade) as vm FROM vekttor_venda $filtro AND revenda_franquia_id = $r->vkt_id"));
			$soma_vlr_implantacao += $valores->vi;
			$soma_vlr_mensalidade += $valores->vm; 
			if($valores_vendas->vi>0){
				$porcentagem_implantacao = ($valores->vi*100)/$valores_vendas->vi;
			}
			if($valores_vendas->vm>0){
				$porcentagem_mensalidade = ($valores->vm*100)/$valores_vendas->vm;
			}
?>      
    	<tr <?=$sel?> id="<?=$r->rf_id?>" onclick="location.href='?tela_id=365&revendedor_id=<?=$r->rf_id?>&data_ini=<?=$data_ini?>&data_fim=<?=$data_fim?>'">
          <td width="50"><?=$r->rf_id?></td>
          <td width="200"><?=$r->razao_social?></td>
          <td width="70"><?php if(!$qtd>0){echo "0";}else{ echo $qtd;}?></td>
          <td width="65"><?php if(!$valores->vi>0){echo "0";}else{ echo $valores->vi;}?></td>
          <td width="65"><?php if(!$porcentagem_implantacao>0){echo "0";}else{ echo $porcentagem_implantacao;}?></td>
          <td width="65"><?=$qtd?></td>
          <td width="65"><?php if(!$valores->vm>0){echo "0";}else{ echo $valores->vm;}?></td>
