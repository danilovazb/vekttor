	...
	<fieldset  id='campos_1' >
	   ...
	   ?>
        <div style="float:right" id="div_foto">
        	<img src="modulos/odonto/servicos/fotos/<?=$servico->id.".".$servico->extensao?>" width="150" height="150"/>
        ...
        <table cellpadding="0" cellspacing="0" style="background-color:white;" width="100%">
        	<thead>
            	<tr>
                	<td width="80%">Convênio</td>
                    <td width="20%">Valor</td>
                </tr>
            </thead>
            <tbody>
            <? 
		$convenios_q=mysql_query("SELECT o.id as id,c.razao_social as nome FROM odontologo_convenio as o, cliente_fornecedor as c WHERE o.vkt_id='$vkt_id' AND o.cliente_fornecedor_id = c.id "); 
		echo mysql_error();
		$i=0;
		while($c=mysql_fetch_object($convenios_q)){
			if($i%2==0){$class='al';}else{$class='';}
			$valor_convenio=@mysql_result(mysql_query("SELECT valor FROM odontologo_procedimento_convenio WHERE vkt_id='$vkt_id' AND servico_id='$servico->id' AND convenio_id='$c->id' "),0,0);
			
		?>
            	<tr class="<?=$class?>">
                	<td><?=$c->nome?></td>
                    <td><input type="text" value="<?=moedaUsaToBR($valor_convenio)?>" name="valor_convenio[<?=$c->id?>]" style="height:10px; width:50px;" sonumero="1" decimal="2"/></td>
                </tr>
		<?
		$i++;
		 } ?>
