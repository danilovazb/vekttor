...
$eleitor_q="SELECT * FROM eleitoral_eleitores WHERE vkt_id='$vkt_id' AND barco_qtd>0 AND barco_servicos!='0' $filtro_servico ";
if($_GET['bairro']){
...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    <?
    	while($eleitor=mysql_fetch_object($eleitor_q)){
			$total+=$eleitor->barco_qtd;
	?>
    	<tr>
        	<td width="220"><?= $eleitor->nome?></td>
            <td width="100"><?= $eleitor->barco_qtd?></td>
            <td><?= $eleitor->barco_servicos?></td>
        </tr>
    <?
		}
	?>
