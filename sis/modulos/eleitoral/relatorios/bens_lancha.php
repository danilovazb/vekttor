...
$(document).ready(function(){
		$("#lanchas tr:nth-child(2n+1)").addClass('al');
...
<div id="barra_info">
    ...
    <label>Serviços<input name="servicos[]" value="servicos" type="checkbox" <? if(@in_array('servicos',$_GET['servicos'])){echo "checked='checked'";}?> /></label>
       <input type="hidden" name="tela_id" value="158" />
...
if(isset($_GET['servicos'])){
	foreach($_GET['servicos'] as $servico){
		$filtro_servico.=" AND lancha_servicos LIKE '%".$servico."%' ";
	}
}
$eleitor_q="SELECT * FROM eleitoral_eleitores WHERE vkt_id='$vkt_id' AND lancha_qtd>0 AND lancha_servicos!='0' $filtro_servico ";
if($_GET['bairro']){
...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    <?
    	while($eleitor=mysql_fetch_object($eleitor_q)){
			$total+=$eleitor->lancha_qtd;
	?>
    	<tr>
        	<td width="220"><?= $eleitor->nome?></td>
            <td width="100"><?= $eleitor->lancha_qtd?></td>
            <td><?= $eleitor->lancha_servicos?></td>
        </tr>
    <?
		}
	?>
...
<table cellpadding="0" cellspacing="0" width="100%;">
<thead>
	<tr><td width="220">Total</td><td width="100"><?=$total?></td><td></td></tr>
</thead>
</table>
</div>
</div>
