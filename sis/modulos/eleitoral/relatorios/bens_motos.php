...
$(document).ready(function(){
		$("#motos tr:nth-child(2n+1)").addClass('al');
...
<div id="barra_info">
    ...
    <label>Serviços<input name="servicos[]" value="servicos" type="checkbox" <? if(@in_array('servicos',$_GET['servicos'])){echo "checked='checked'";}?> /></label>
       <input type="hidden" name="tela_id" value="161" />
...
if(isset($_GET['servicos'])){
	foreach($_GET['servicos'] as $servico){
		$filtro_servico.=" AND moto_servicos LIKE '%".$servico."%' ";
	}
}

$eleitor_q="SELECT * FROM eleitoral_eleitores WHERE vkt_id='$vkt_id' AND moto_qtd>0 AND moto_servicos!='0' $filtro_servico ";
if($_GET['bairro']){
...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    <?
    	while($eleitor=mysql_fetch_object($eleitor_q)){
			$total+=$eleitor->moto_qtd;
	?>
    	<tr>
        	<td width="220"><?= $eleitor->nome?></td>
            <td width="100"><?= $eleitor->moto_qtd?></td>
            <td><?= $eleitor->moto_servicos?></td>
        </tr>
    <?
		}
	?>
