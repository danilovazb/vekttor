<?php
	...
	$file = fopen("promocao_natal2012.csv",'r');
	global $vkt_id;
	while(!feof($file)){
		$linha = fgets($file);
		...
		if(!($existe_email->id>0)){
			mysql_query($t="INSERT INTO eleitoral_eleitores SET vkt_id='169',nome='$nome',email='$email',data_nascimento='$data_nascimento',telefone1='$telefone1',sexo='$sexo',cep='$cep',endereco='$endereco', bairro='$bairro', cidade='$cidade', descricao_bens='$obs', grupo_social_id='29', origem='Programa Josue Neto'");
		    echo $t."<br>";
			$id=mysql_insert_id();
		}else{
			$id=$existe_email->id;
		...
		$promocao_id='3';
		$existe_promocao = mysql_fetch_object(mysql_query($t="SELECT * FROM eleitoral_participante_promocao WHERE eleitor_id='$id' AND promocao_id='$promocao_id' AND vkt_id='169'"));
		echo $t."<br>";
		if($existe_promocao->id<=0){
			mysql_query($t="INSERT INTO eleitoral_participante_promocao SET promocao_id='$promocao_id', eleitor_id='$id', vkt_id='169'");
			echo $t."<br>";
		}
