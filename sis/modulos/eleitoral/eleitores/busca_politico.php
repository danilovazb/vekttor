...
$q=mysql_query("SELECT * FROM eleitoral_politicos WHERE vkt_id ='$vkt_id' AND (nome  like '%$_GET[busca_auto_complete]%')  LIMIT 15");
$i=0;
while($r= mysql_fetch_object($q)){
	...
	echo urlencode("$r->nome|$r->cargo|{$partido->sigla}|{$coligacao->nome}|$r->id|\n");
	$i++;
}
if($i==0){
	echo urlencode("Não Encontrado, Cadastre em Político|0|0\n");
}
?> 