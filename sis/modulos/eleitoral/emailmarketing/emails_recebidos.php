...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    $registros= mysql_result(mysql_query("SELECT count(*) FROM eleitoral_emailmarketing_retorno WHERE vkt_id = '$vkt_id' AND email_marketing_id = '$id' $busca_add $filtro"),0,0);
	...
	$q= mysql_query($t="SELECT *,date_format(data,'%d/%m/%Y as %m:%i') as dt FROM 
		eleitoral_emailmarketing_retorno
		WHERE
