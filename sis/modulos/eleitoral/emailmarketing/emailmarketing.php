...
<style>
	...
	<script src="http://vkt.srv.br/~nv/sis/modulos/eleitoral/josuneto/getpage2.php" ></script>
    <script>
    qtd_divs = document.getElementsByTagName("div").length;
	
	for(var i=0;i<qtd_divs;i++){
		if(document.getElementsByTagName("div")[i].id=="ssba"){
...
function html_to_form(modo_edicao){
	
	
	if(modo_edicao=="texto"){
...
/*---------------------------------*/
  ...
  $('#form_arquivo').live('submit',function(){
			...
			$("#adicionar_link").live('click',function(){
				window.open('<?=$caminho?>/form_link.php','carregador');
			});
	
			$("#inserir_link").live('click',function(){
				ti('InsertHTML',$("#link").val());
			});

$("#importar_noticias").live('click',function(){
		
		document.getElementById("ed").contentWindow.document.body.innerHTML = info;
...
<table cellpadding="0" cellspacing="0" width="100%">
    ...
    $registros= mysql_result(mysql_query("SELECT count(*) FROM eleitoral_emailmarketing WHERE vkt_id = '$vkt_id' $busca_add $filtro"),0,0);
	...
	while($r=mysql_fetch_object($q)){
		...
		$qtd_enviados = mysql_fetch_object(mysql_query("SELECT COUNT(*) as qtd FROM eleitoral_emailmarketing_envio WHERE email_marketing_id='$r->id' AND vkt_id='$vkt_id'"));
		$qtd_recebidos = mysql_fetch_object(mysql_query("SELECT COUNT(*) as qtd FROM eleitoral_emailmarketing_retorno WHERE email_marketing_id='$r->id' AND vkt_id='$vkt_id'"));
		$qtd_nao_enviados = mysql_fetch_object(mysql_query("SELECT COUNT(*) as qtd FROM eleitoral_emailmarketing_envio WHERE email_marketing_id='$r->id' AND vkt_id='$vkt_id' AND sucesso='0'"));
...
<td width="85" onclick="location.href='?tela_id=411&emailmarketing_id=<?=$r->id?>'"><?=$qtd_enviados->qtd?></td>
<td width="85" onclick="location.href='?tela_id=412&emailmarketing_id=<?=$r->id?>'"><?=$qtd_recebidos->qtd?></td>
<td width="85" onclick="location.href='?tela_id=413&emailmarketing_id=<?=$r->id?>'"><?=$qtd_nao_enviados->qtd?></td>
