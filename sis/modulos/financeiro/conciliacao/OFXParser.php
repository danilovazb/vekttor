<?php
	class OFXParser
	{
		...
		public function loadFromString($OFXContent)
		{
			...
			$fi = sizeof(explode('<FI>', $this->OFXContent));
			...
			$this->fixTag('BALAMT');
			$this->fixTag('LEDGERBAL');
			//Adicionado para Santander
			if($fi>1){
				$this->fixTag('ORG');
				$this->fixTag('FID');
				$this->fixTag('PAYEEID');
				$this->fixTag('TRNAMT',true);
				$this->fixTag('STMTTRN',true);
				$this->fixTag('BALAMT',true);
			}
...
', trim($this->conf));
		...
		public function getCredits($min= 0)
		{
			...
			for($i=0, $j= sizeof($this->moviments); $i<$j; $i++)
			{
				//echo "Credito: ".$this->moviments[$i]['TRNAMT']."<br>";
				if(($this->moviments[$i]['TRNTYPE'] == 'CREDIT' && $this->moviments[$i]['TRNAMT'] >= $min)||($this->moviments[$i]['TRNAMT'] >= $min)){
		...
		public function getDebits($min= 0)
		{
			...
			for($i=0, $j= sizeof($this->moviments); $i<$j; $i++)
			{
				//echo "Débito: ".$this->moviments[$i]['TRNAMT']."<br>";
				if(($this->moviments[$i]['TRNTYPE'] == 'DEBIT' && $this->moviments[$i]['TRNAMT'] <= $min)||($this->moviments[$i]['TRNAMT'] <= $min))
