...
if($_GET[data_informativo]){$campo_data=$_GET[data_informativo];}
...
$(".botao_imprimir").live('click',function(){
	$(".coluna1").width('');
	$(".coluna1").css("width","595");
	$(".entrada").css("width","60");
	$(".coluna_movimentacoes").css("font-size","10px");
	window.open('modulos/tela_impressao.php?url=<? $url?>');
});
</script>
<!-- -->
...
<div id="barra_info">
<form style=" margin:0; padding:0; " method="get" id="form_filtro"> 
...
<input style="margin-left:-2px;" type="checkbox" <? if($_GET['extorno']==1)echo "checked='checked'"; ?> name="extorno" value="1" />
    ...
    <select name="data_informativo" title='Ordenação na exibição' rel='tip'>
      <option value="data_vencimento" <? if($_GET[data_informativo]=='data_vencimento'||!$_GET[data_informativo])echo "selected"; ?>>Vencimento</option>
      <option value="data_info_movimento" <? if($_GET[data_informativo]=='data_info_movimento')echo "selected"; ?>>Pagamento</option>
      <option value="data_movimento" <? if($_GET[data_informativo]=='data_movimento')echo "selected"; ?>>Efetivação</option>
    </select>
...
<div id='dados' style="height:529px;">
<div id="info_filtro">
...
<table cellpadding="0" cellspacing="0" width="100%" >
    ...
    <?		
			...
			if(!isset($_GET[tipo]) || $_GET[tipo]=='nulo' || $_GET[tipo]=='centro' || $_GET[tipo]=='plano'){
			...
			if($_GET[tipo]=='cliente'||$_GET[tipo]=='nota'||$_GET[tipo]=='ordem' ){
				...
				$q = mysql_query($t="
					...
					FROM
						 financeiro_movimento as m,
						 (SELECT * FROM financeiro_centro_custo WHERE cliente_id='$vkt_id' AND plano_ou_centro='plano') as tabela_plano,
						 (SELECT * FROM financeiro_centro_custo WHERE  cliente_id='$vkt_id' AND plano_ou_centro='centro') as tabela_centro,
			...
			while($r=mysql_fetch_object($q)){
				...
				if( $_GET[tipo] != 'cliente' ){
					...
					$oposto_query=mysql_query($op="SELECT c.nome FROM financeiro_{$oposto}_has_movimento as h, financeiro_movimento as m, financeiro_centro_custo as c 
						WHERE m.id='{$r->id}' 
						...
						switch($oposto){
							case'centro':
							$centro=mysql_fetch_object($oposto_query);
							break;
							case'plano':
							$plano=mysql_fetch_object($oposto_query);
							break;
						}
				}
    	...
    	<tr <?=$cl?> <?=$v?> onClick="opf(<?=$r->id?>)" >
            ...
            <td width="70"><? if($r->data_mov=="0000-00-00"){echo "Pendente";}else{echo dataUsaToBr($r->data_mov); }?></td>
          	<td width="100" class="coluna_movimentacoes"><? if($oposto=='plano'){echo $r->nome; }else{ echo $centro->nome;}?> <? if($_GET[tipo]=='cliente')echo $r->centro_nome; ?></td>
            
            <td width="100" class="coluna_movimentacoes"><? if($oposto=='centro'){echo $r->nome; }else{ echo $plano->nome; }?> <? if($_GET[tipo]=='cliente')echo $r->plano_nome; ?></td>
            
            <td width="100" align="left" class="coluna_movimentacoes"><?=$cliente->nome?></td>
            <td width="150" align="left" class="coluna_movimentacoes"><?php echo substr($r->descricao,0,50)?></td>
...
<table cellpadding="0" cellspacing="0" width="100%">
  <thead>
    	<tr>
       	  <td width="645" class="coluna1">Total</td>
          
          	<td width="70" style="margin:0; padding:0; text-align:right" class="entrada"><?=number_format($total_entrada,2,',','.')?></td>
          	<td width="60" class="saida"><?=number_format($total_saida,2,',','.')?></td>
          	<td width=""></td>
      </tr>
    </thead>
</table>
...
<table cellpadding="0" cellspacing="0"width="100%" >
  <thead>
    <tr>
      <td width="645" class="coluna1">Pago</td>
          
          <td width="70" style="margin:0; padding:0; text-align:right" class="entrada"><?=number_format($total_pago_entrada,2,',','.')?></td>
          	<td width="60" class="saida"><?=number_format($total_pago_saida,2,',','.')?></td>
          	<td width=""></td>
    </tr>
  </thead>
</table>
...
<table cellpadding="0" cellspacing="0" width="100%">
  <thead>
    <tr>
      <td width="645" class="coluna1">Pendente</td>
       
          	<td width="70" style="margin:0; padding:0; text-align:right" class="entrada"><?=number_format($total_pendente_entrada,2,',','.')?></td>
          	<td width="60" class="saida"><?=number_format($total_pendente_saida,2,',','.')?></td>
          	<td width=""></td>
    </tr>
  </thead>
</table>
<table cellpadding="0" cellspacing="0" width="100%">
  <thead>
    <tr>
      <td width="645" class="coluna1">Lucro</td>
       <?
	   
	   	if($total_entrada > 0){
			$lucro = ($total_entrada)*1 - ($total_saida) *1;
	   ...
	   ?>
          	<td width="70" style="margin:0; padding:0; text-align:right" class="entrada"></td>
          	<td width="60" class="saida"><?=number_format(($lucro),2,',','.')?></td>
          	<td width=""></td>
    </tr>
  </thead>
</table>
