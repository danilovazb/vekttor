...
.tabela_plano_contas table,.tabela_centro_custo table { border-left:1px solid #999; border-collapse:collapse;}
</style>
...
$(".retira_item_plano").live("click",function(){
	var plano_id = $(this).parent().parent().attr("id");
	
	$("div#plano_de_contas div.plano_"+plano_id+"").remove();
	
	var j = 0;
	var tbody = $(this).closest("div");
	var tr = $(this).parent().parent().parent().find("tr");	
...
$(".add-novo-plano").live("click",function(){ // Adiciona o item na tabela Plano de Contas
	
	var plano_id = $("#plano_de_conta_id_prepare").val();
	
	if( $.trim(plano_id) != "" ){
		var plano_name = $("#plano_conta").val();
		var td = "";
		var tr = "<tr id="+plano_id+">";
		var tr_fim = "</tr>";
			
			/* Insere dentro do DIV com id (plano_de_contas) para montar o array */
			$("div#plano_de_contas").append('<div class="plano_'+plano_id+'"><input type="hidden" name="plano_de_conta_id[]" id="plano_de_conta_id" value="'+plano_id+'"></div>'); 
		
		td += '<td>'+plano_name+'</td>';
		td += '<td><img class="retira_item_plano" src="../fontes/img/menos.png"></td>';
		
		$(".tabela_plano_contas").show();
		
		$(".lista_plano_contas").append(tr+td+tr_fim);
		$(".lista_plano_contas tr:odd").addClass('al');
		$("#plano_de_conta_id_prepare").val("");
		$("#plano_conta").val("");
	} else {
		$("#plano_conta").val(" Informe plano de contas aqui...");
	}
	
});//

/**/
$("#plano_conta").live("focus",function(){
	$("#janela_centro_custo").hide();
	$("#janela_plano_contas").show();
	$("#plano_conta").val("");
...
$("#click_plano_contas").live("click",function(){
	...
	$("#plano_de_conta_id_prepare").val(plano_id);
...
/* Centro de Custo */
$(".retira_item_centro").live("click",function(){ // Exclui o item da tabela Centro de Custos
	
	var centro_id = $(this).parent().parent().attr("id");
	
	$("div#centro_de_custos div.centro_"+centro_id+"").remove();
...
$(".add-novo-centro").live("click",function(){ // Adiciona o item na tabela Centro de Custos
	var centro_id = $("#centro_custo_id_prepare").val();
	
	if( $.trim(centro_id) != "" ){
		var centro_name = $("#centro_custo").val();
		var td = "";
		var tr = "<tr id="+centro_id+">";
		var tr_fim = "</tr>";
		
			/* Insere dentro do DIV com id (centro_de_custos) para montar o array */
			$("div#centro_de_custos").append('<div class="centro_'+centro_id+'"><input type="hidden" name="centro_custo_id[]" id="centro_custo_id" value="'+centro_id+'"></div>'); 
		
		td += '<td><input type="hidden" name="centro_custo_id[]" id="centro_custo_id" value="'+centro_id+'">'+centro_name+'</td>';
		td += '<td><img class="retira_item_centro" src="../fontes/img/menos.png"></td>';
		
		$(".tabela_centro_custo").show();
		
		$(".lista_centro_custo").append(tr+td+tr_fim);
		$(".lista_centro_custo tr:odd").addClass('al');
		$("#centro_custo_id_prepare").val("");
		$("#centro_custo").val("");
	} else {
		$("#centro_custo").val(" Informe centro de custo aqui...");
	}
...
/* abre janelas centro de custos */
$("#centro_custo").live("focus",function(){
	$("#janela_plano_contas").hide();
	$("#janela_centro_custo").show();
	$("#centro_custo").val("");
	$("#janela_centro_custo .input_centro ").focus();
...
$("#click_centro_custo").live("click",function(){
	...
	$("#centro_custo_id_prepare").val(centro_id);
