...
function exibe_option_sub_plano_ou_centro_1($plano_ou_centro,$pai_id,$id_do_selecionado,$nivel,$pai_ordem=null){
	global $vkt_id;
	global $busca;
  $pai = mysql_fetch_object(mysql_query("SELECT * FROM financeiro_centro_custo WHERE id='$pai_id' AND nome LIKE '%$_GET[busca_auto_complete]%'  "));
  ...
  $q= mysql_query($r="SELECT * FROM financeiro_centro_custo 
						...
						AND 
							nome LIKE '%$_GET[busca_auto_complete]%' AND cliente_id = '$vkt_id'
...
exibe_option_sub_plano_ou_centro_1('plano',0,0,0);	
