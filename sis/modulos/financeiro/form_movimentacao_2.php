	...
	<fieldset  id='campos_1' >
		...
		<label style="width:100px;">
			Forma 
			  ...
			  ?>
                ...
                <option value="7" <?=$select_pagamento[7]?>>Transferencia</option>
                <option value="8" <?=$select_pagamento[8]?>>Depósito</option>
                <option value="3" <?=$select_pagamento[3]?>>Cartao Crédito Visa</option>
                <option value="9" <?=$select_pagamento[9]?>>Cartao Crédito Mastar</option>
                <option value="10" <?=$select_pagamento[10]?>>Débito Mastar</option>
                <option value="11" <?=$select_pagamento[11]?>>Débito Visa</option>
		...
		<label style="width:100px;">
			Valor
              ...
              " valida_minlength='3' style="text-align:right;background:#BAE7BE" retorno='focus|Valor Incorreto'  <?=$desabilita_finalizado?> onfocus="document.getElementById('calendario').style.display='none'">
        </label>
...
<div style="float:left; width:350px; border:0px solid #333;" id='centro_de_custos'>
	<?
   ...
   <div class='listacentro' id='listacentro_<?=$list_cont?>' style="clear:both; display:block">     
      ...
      <label id="array_centro_id" style="position:absolute; display:none;">
          <input type="text" name="centro_custo_id[]"  style="width:80px;" value="<?=$rcp->plano_id?>">
      </label>
      ...
      <label style="width:190px; margin-right:0;">Centro de Custos
          <input type="text" name="centro_custo[]" id="centro_custo" style="width:180px;" value="<?=$centro->nome?>">
      </label>
        
        <!-- modal centro de custos -->	
           <div style="position:absolute; margin-left:-25px;"  class="modal_centro_custo">
           	  <span id="insercao" style="display:none">listacentro_<?=$list_cont?></span>
              <div class="sub-janela" style="display:none;" id="janela_centro_custo">
              <div class="modal-header-2">
              	<a href="#" style="color:#CCC; font-weight:bold;float:right;" class="modal_close">x</a>
                <span>Centro de Custos</span>
              </div>
                    <div class="modal-body">
                    	<div><label><input type="text" class="input_centro" modal="janela_centro_custo" name="busca_plano" id="busca_plano" index='-1'></label></div>
                        <div style="clear:both;"></div>
                    	<div style="overflow:auto; max-height:200px;">
                        	<table cellpadding="0" cellspacing="0" style="border-left:1px solid #ccc;border-right:1px solid #ccc;" width="100%" class="table-dados" >
                              <thead>
                                <tr>
                              ...
                              <tbody>
                              <?
								exibe_option_sub_plano_ou_centro2_1('centro',0,$rcp->plano_id,0);	
							  ?>
                               </tbody>
						  </table>  
                      </div>
                   </div>
                   <div style="" id="result-modal"></div>
        ...
        <!-- fim modal -->  
 		...
 		<label style="width:75px; margin-right:0;">
			Valor <input name="centro_valor[]" id="centro_valor" style="width:65px;" onkeyup="calc_plano_centro_valor(this)" value="<?=moedaUsaToBr($rcp->valor) ?>"    > <!-- onkeyup="calc_plano_centro_valor(this)" -->  
       </label>
  		<label style="width:30px;">
        %
         <input name="centro_porcentagem[]" id="centro_porcentagem" onkeyup="calc_plano_centro_porcentagem(this)"  value="<?=$porcentagem?>"  > <!-- onkeyup="calc_plano_centro_porcentagem(this)" -->
       </label>
      <!--<img src="../fontes/img/<?=$img?>.png" onclick="addPlc(this)" width="18" height="18" style=" margin-top:20px" />-->
      
      <button type="button" onclick="addPlc(this)" style="font-weight:bold; margin-top:18px;">+</button>
    </div>
    <?
	$list_cont++;
	}
	...
	<div style="float:left; width:350px; border:0px solid #666;" id='plano_de_contas'>
        <?
	...
	?>
        ...
        <label id="array_plano_id" style=" position:absolute; display:none;">
        	<input type="text" name="plano_de_conta_id[]" id="plano_de_conta_id" style="width:80px;" value="<?=$rcp->plano_id?>">
        </label>
        ...
        <label style="width:190px; margin-right:0; ">Plano de Conta
        	<input type="text" name="plano_conta" id="plano_conta" style="width:180px;" autocomplete='off' value="<?=$plano->nome?>">
        </label> 
        
        <!-- modal plano de contas -->	
           <div style="position:absolute;" class="modal_plano_contas" >
              <div class="sub-janela" style="display:none" id="janela_plano_contas">
              <span id="insercao" style="display:none">listaplano_<?=$list_plano?></span>
              <div class="modal-header-2">
              	<a href="#" style="color:#CCC; font-weight:bold;float:right;" class="modal_close">x</a>
                <span>Plano de Contas</span>
              </div>
                    <div class="modal-body" >
                     <div><label><input type="text" name="busca_plano" modal="janela_plano_contas" index='-1' id="busca_plano" class="busca_plano_de_contas"></label></div>
                        <div style="clear:both;"></div>
                    	<div style="min-height:200px; overflow:auto;">
                          
                          <table  width="100%" cellpadding="0" cellspacing="0" style="border-left:1px solid #ccc;border-right:1px solid #ccc;" class="table-dados" >
                          <thead>
                                <tr>
                                	<td>Planos de contas</td>
                                    <td></td>
                                </tr>
                              <tbody>
                              <?
								exibe_option_sub_plano_ou_centro_1('plano',0,$rcp->plano_id,0);	
							  ?>
                               </tbody>
						  </table>
       ...
       <!-- fim modal -->  
        ...
        <label style="width:75px; margin-right:0;">
			Valor
			  <input name="plano_valor[]" id="plano_valor" style="width:65px;" onkeyup="calc_plano_centro_valor(this)" value="<?=moedaUsaToBr($rcp->valor)?>" > <!-- onkeyup="calc_plano_centro_valor(this)" -->
       </label>
  		<label style="width:30px;">
        %
         <input name="plano_porcentagem[]" id="plano_porcentagem" onkeyup="calc_plano_centro_porcentagem(this)" value="<?=$porcentagem?>" > <!-- onkeyup="calc_plano_centro_porcentagem(this)" -->
       </label>
      <!--<img src="../fontes/img/<?=$img?>.png" onclick="addPlc(this)" width="18" height="18" style=" margin-top:20px" />--> 
