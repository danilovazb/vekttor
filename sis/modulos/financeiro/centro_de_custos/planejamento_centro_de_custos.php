...
?>
<style>
.pln input{ border:0; background:none; text-align:right; width:44px;}
	...
	while($r=mysql_fetch_object($q)){
		...
		for($mes=0;$mes<12;$mes++){
			...
			$total_no_mes[$mes]+=$planejado->valor;
			 ...
			 ?>
			  <td width="63" align="right" title="<?=$planejado->valor?>" ><input <?=$dis?> class="registro" onblur="atualizaDado(<?=$r->id?>,this,<?=$mes_q?>,<?=$ano?>)" type="text" value="<?=number_format($planejado->valor,2,',','.')?>"  /></td> 	
			<?
		}
...
<table cellpadding="0" cellspacing="0" width="100%">
    <thead>
    	<tr id="linha_total">
            <td width="209"><span style="min-width:150px; overflow:hidden;">Total</span></td>
            <td width="98"align="right"><?=$total_no_mes[0]?></td>
            <td width="98"align="right"><?=$total_no_mes[1]?></td>
            <td width="98"align="right"><?=$total_no_mes[2]?></td>
            <td width="98"align="right"><?=$total_no_mes[3]?></td>
            <td width="98"align="right"><?=$total_no_mes[4]?></td>
            <td width="98"align="right"><?=$total_no_mes[5]?></td>
            <td width="98"align="right"><?=$total_no_mes[6]?></td>
            <td width="98"align="right"><?=$total_no_mes[7]?></td>
            <td width="98"align="right"><?=$total_no_mes[8]?></td>
            <td width="98"align="right"><?=$total_no_mes[9]?></td>
            <td width="98"align="right"><?=$total_no_mes[10]?></td>
<td width="98"align="right"><?=number_format(@array_sum($vtdss),2,',','.')?></td>
