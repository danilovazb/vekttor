...
function gerencia_contrato($contrato_id,$campos,$action){
	...
	//echo "$sql_inicio $sql_campos $sql_fim <br>";
	if($contrato_id<1){ 
...
function contrato_movimento_deleta($contrato_id){
	...
	$q =mysql_query($t="SELECT * FROM financeiro_movimento WHERE origem_id='$contrato_id' AND origem_tipo='Contrato' AND cliente_id='$vkt_id' AND status!='2'");
	//echo $t."<br>";
	while($r= mysql_fetch_object($q)){
		//echo $r->id."<br>";
		mysql_query($t="DELETE FROM financeiro_plano_has_movimento WHERE movimento_id ='$r->id'");
		//echo $t." <br>";
		mysql_query($t="DELETE FROM financeiro_centro_has_movimento WHERE movimento_id ='$r->id'");
		//echo "$t <br/>";
		mysql_query($t="DELETE FROM financeiro_movimento WHERE id='$r->id'");		
		//echo "$t <br/>";
