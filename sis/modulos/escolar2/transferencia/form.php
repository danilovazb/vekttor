      ...
      <fieldset id="campos_2" style="display:none" >
            ...
            <?
				...
				while($periodo_letivo=mysql_fetch_object($sql_periodo_letivo)){
					if($mat_atual->periodo_id==$periodo_letivo->id){
						...
						$periodo_selecionado = 	$periodo_letivo->id;
					}
			?>
            	...
            	<?
					...
					while($unidade=mysql_fetch_object($sql_unidade)){
						...
						if($matricula_turma->unidade_id==$unidade->id){
							$selected="selected='selected'";
							$unidade_selecionada = 	$unidade->id;
						}
				?>
        ...
        <label style="width:100px;">Série
        	<select name="serie_transferencia" id="serie_transferencia">
           		<option value="0">Selecione</option>
            	<?
            		$sql_count = mysql_query($y1=" SELECT DISTINCT sala_id,id FROM escolar2_turmas WHERE periodo_letivo_id = '$periodo_selecionado' AND unidade_id = '$unidade_selecionada'");
			...
			while($turmaCount=mysql_fetch_object($sql_count)){
					...
					$sala  = mysql_fetch_object(mysql_query($t=" SELECT * FROM escolar2_salas WHERE id = '$turmaCount->sala_id' "));
		   ...
		   if( $vagas_disponiveis == 0 ){
				$disabled_salas = "disabled='disabled'";   
		   }
		   
		   
		   $sql = mysql_query($y2=" SELECT DISTINCT serie_id FROM escolar2_turmas WHERE periodo_letivo_id = '$periodo_selecionado' AND unidade_id = '$unidade_selecionada'");
		  ...
		  while($turma=mysql_fetch_object($sql)){
			  ...
			  if($mat_atual->serie_id==$serie->id){
			  
				$selected="selected='selected'";
				$serie_selecionada = $serie->id;
			  }
			  
			 echo " <option value='".$serie->id."' $disabled_salas $selected>".utf8_encode($serie->nome)." - Vaga: ".$vagas_disponiveis."</option> ";
			 $selected='';
		  }
           ?>
            </select>
        ...
        <label style="width:100px;">Turma
      	<select name="turma_transferencia" id="turma_transferencia">
            ...
            <option value="0">Selecione</option>
            	<?
            		$sql_turma = mysql_query(" SELECT * FROM escolar2_turmas WHERE periodo_letivo_id = '$periodo_selecionado' AND unidade_id = '$unidade_selecionada' AND serie_id = '$serie_selecionada' ORDER BY nome ASC");
					while($turma=mysql_fetch_object($sql_turma)){
						echo $mat_atual->turma_id;
			...
			<select name="modelo_id" id="modelo_id" retorno="focus|Selecione o Modelo do Contrato" valida_minlength='1'>
				...
				<?php
					...
					while($modelo = mysql_fetch_object($modelos)){
						//alert($modelo->id." - ".$matricula_aluno->modelo_contrato_id);
						if($modelo->id == $matricula_aluno->modelo_contrato_id){
							$selected="selected='selected'";
						}
...
<div style="clear:both"></div>
      ...
      <input type="button" id="confirmar_matricula" name="action"  value="Salvar" style="float:right; "/>
