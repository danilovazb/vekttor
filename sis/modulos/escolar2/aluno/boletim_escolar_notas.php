...
.tbl_header_1 th, .tbl_header_2 th{padding:3px 8px; border:0;border-bottom:1px solid #999; }
table{border-collapse:collapse;}
table tr td{font-size:8pt; border:1px solid #666;}
table tr th{font-size:9pt; border:1px solid #666;}
/*td{padding:4px; padding-left:3px}
.table-content tr td{border:1px solid #999; border-right:1px solid #999; font-size:10px; font-weight:bold}
.table-content tr, .table-content th{border-top:1px solid #666; border-left:1px solid #666;border-right:1px solid #666;font-size:10pt;}*/
</style>
</head>
       ...
       <div style="min-height:250px;"  class="content-table">
          ...
          <tr align="center" class="cabecalho">
              ...
              <? for($i=0;$i<count($periodo_avaliacao);$i++){ 
                  $count_av = count(retorna_avaliacao_periodo(NULL,NULL,$periodo_avaliacao[$i]["periodo_av"]));
              ?>
              <th colspan="<?=$count_av?>"> <?php echo $periodo_avaliacao[$i][nome]?> </th>
              <? } ?>
           ...
           <tr align="center" >
              <? for($i=0;$i<count($periodo_avaliacao);$i++){ 
                  $avaliacoes = retorna_avaliacao_periodo(NULL,NULL,$periodo_avaliacao[$i]["periodo_av"]);
				  
				  for($j=0;$j < count($avaliacoes);$j++ ){
              ?> 
              <td> <?=$avaliacoes[$j]["nome"]?> </td>
          
			  <? } 
			  } ?>
              <td width="80"> Média Final </td>
              <td width="80"> Resultado Final </td>                             
          </tr> 
		  ...
		  ?>
           <tr>
                ...
                <? for($i=0;$i<count($periodo_avaliacao);$i++){  
					...
					for($h=0; $h <count($avaliacoes);$h++){
						$nota = retorna_nota_avaliacao($avaliacoes[$h]["id"],$info_header->matricula_aluno,$info_serie_materia[$s]["materia_id"]);
							
				?>
                <td align="center"><? if( !empty($nota) ) echo number_format($nota,1,",",".")?></td>
                <?  }
				  }
				?>
