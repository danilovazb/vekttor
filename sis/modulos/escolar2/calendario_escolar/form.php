...
<div>
	...
	<fieldset  id='campos_1' >
        ...
        <!--<label style="width:262px;">Cliente
        	<input type="text" name="nome_cliente" id="cliente" value="<?$cliente->nome_fantasia;?>" busca="modulos/agendamento/agenda_diaria/busca_cliente.php,@r1,@r1-value>nome_cliente|@r0-value>cliente_id,0">
        </label>-->
        ...
        <!--<label style="width:250px;"> Agenda
        	<select name="agenda_id" id="agenda_id">
              	<option value="0" >Agenda</option>
          	<?php 
					$sqlAgenda = mysql_query(" SELECT * FROM agenda WHERE vkt_id = '$vkt_id' AND usuario_id = '$usuario_id'");
						while($r_agenda=mysql_fetch_object($sqlAgenda)){
							 if($agenda->id == $r_agenda->id) { $sel_option = "selected='selected'";}else{ $sel_option = "";}
				?>
            	<option value="<?php echo $r_agenda->id?>" <?=$sel_option;?>><?php echo $r_agenda->nome?></option>
            	<?php } ?>
            </select>
        </label>-->
        <label style="width:250px;">Evento
          <textarea name="evento" id="evento"><?=$agendamento->nota_adicional;?></textarea>
		</label>
        <label style="width:80px;">Data Inicio
        	<input type="text" name="data_inicio" id="data_inicio" readonly="readonly" value="<?php if($data_inicio){ echo dataUsaToBr($data_inicio); } else { echo $dataEvento;} ?>">
        </label>
        <label style="width:65px;">Hora Inicio
        	<input type="text" name="hora_inicio" readonly="readonly" maxlength="5" id="hora_inicio" value="<?php if($hora_inicio){ echo substr($hora_inicio,0,5);} else{echo $hora_inicial;}?>">
        </label>
        <div style="clear:both;"></div>
        <label style="width:80px;">Data Fim
           <input type="text" name="data_fim" id="data_fim" readonly="readonly" value="<?php if($data_fim){ echo  dataUsaToBr($data_fim);}else{echo $dataFinal;}?>">
        </label>
        <label style="width:58px;">Hora Fim
        	<input type="text" name="hora_fim" id="hora_fim" readonly="readonly" value="<? if($hora_fim) {echo substr($hora_fim,0,5); } else {echo $hora_final;}?>">
        </label>
	</fieldset>
