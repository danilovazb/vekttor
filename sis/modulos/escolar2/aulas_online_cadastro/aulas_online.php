	  ...
	  <script>resize()</script><!-- Isso é Necessário para a criação o resize -->
        <table cellpadding="0" cellspacing="0" width="100%">
			...
			if(!empty($_GET['periodo_id'])){
				$filtro_p="AND escolar2_periodos.id=".$_GET['periodo_id'];
			}
			
			 $s_periodo = (mysql_query($p=" SELECT DISTINCT(p.periodo_id), p.nome
			 								FROM escolar2_periodo_letivo as p, escolar2_professor_has_turma as ept, escolar2_turmas as et
											WHERE 
												p.vkt_id='$vkt_id'	
	  										AND 
												et.periodo_letivo_id=p.id
	  										AND 
												ept.turma_id = et.id
											AND 
