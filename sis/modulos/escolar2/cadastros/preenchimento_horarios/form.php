...
<div>
	...
	<fieldset  id='campos_1' >
        ...
        <label>
            ...
            <?
            $materia_q=mysql_query($t="
			SELECT 
				esm.id as id, em.nome as nome
			FROM 
				escolar2_serie_has_materias as esm, escolar2_materias as em
			
			WHERE 
				esm.vkt_id='$vkt_id' 				
			AND 
				esm.serie_id='{$turma->serie_id}' 
			AND 
		 ...
		 ?>
         	<table cellpadding="0" cellspacing="0" style="width:100%;border-left:1px solid #CCC;">
                ...
                <tbody>
					...
					?>
                    	<tr style="background:white;" class=" <?=$c?> form_horario ">
                        <td><?=$i+1?>º</td>
                        	<? 
                        	...
                        	<td>
                                ...
                                if($materias_horarios_professor[$dia][$i]){
									echo $materias_horarios_professor[$dia][$i];
                                }else{
								?>
