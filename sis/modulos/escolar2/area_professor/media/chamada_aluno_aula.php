...
<div id="barra_info">
	...
	<span><button type="button" onclick="location.href='?tela_id=259&materia=<?=$_SESSION['materia_id']?>&periodo_materia=<?=$_SESSION['periodo_materia']?>&sala=<?=$sala?>'">&laquo;</button></span> 
...
<table cellpadding="0" cellspacing="0" width="100%">
    <tbody>
    	<tr>
            <td colspan="2" align="center"><strong>SELECIONAR TODOS</strong></td>
    	...
    	<?
        	while($result_mat=mysql_fetch_object($sql_sala)){
					...
					$aluno_presenca = mysql_fetch_object(mysql_query(" SELECT * FROM escolar_frequencia_aula 
																			WHERE aula_id = '$aula' 
																			AND matricula_aluno_id = '$result_mat->id'"));
		?>
	    	<tr>
               <td width="60"><?=$result_mat->id?></td>
               <td width="200"><?=$aluno->nome?></td>
   			   <td width="65" class="mudar"><input type="checkbox" <? if($aluno_presenca->presenca == '1'){echo 'checked="checked"';}?>  matricula="<?=$result_mat->id?>" name="presenca[]" id="presenca"></td>
               <td></td>
            </tr>
         <?
			}
