...
<table cellpadding="0" cellspacing="0" width="100%">
    <tbody>
    	<?
        	...
        	$alunos_recuperacao = mysql_query($rec=" SELECT * FROM escolar_notas AS en
          												JOIN 
															escolar_alunos AS ea ON en.matricula_aluno_id=ea.id
														WHERE 
															en.avaliacao_id = '$avaliacao' ");
			
			while($notas=mysql_fetch_object($alunos_recuperacao)){
				
					$aluno = mysql_fetch_object(mysql_query(" SELECT * FROM escolar_alunos WHERE id = '$notas->matricula_aluno_id'"));
		...
		?>
	    	<tr>
               <td width="60"><?=$aluno->id?></td>
               <td width="200"><?=$aluno->nome?></td>
   			   <td width="75"><input type="text" matricula="<?=$aluno->id?>"  sonumero='1' decimal="1"   name="nota_aluno" id="nota_aluno" value="<?=$notas->nota?>" style="width:50px; height:13px;"/></td>
               <td></td>
            </tr>
         <?
			}
