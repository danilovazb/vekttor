...
if(strlen($_GET['filtro'])>0){
	...
	if($if[0]=='curso'){
		$add2.= "AND et.periodo_letivo_id = '{$if[3]}'  AND et.unidade_id='{$if[2]}' AND ee.id='{$if[1]}' ";
		$unidade_id=$if[2];
	}
	if($if[0]=='modulo'){
	...
	$banco=mysql_fetch_object(mysql_query("SELECT *, eu.nome as nome_unidade FROM escolar2_unidades as eu, financeiro_contas as fc WHERE eu.vkt_id='$vkt_id' AND eu.id='$unidade_id' AND fc.id=eu.conta_id"));
}

if($_GET['filtro_matricula']>0){
	$add2.= "AND m.id= '{$_GET['filtro_matricula']}'  ";
	$banco=mysql_fetch_object(mysql_query($a="SELECT fc.codigo_banco as codigo_banco FROM escolar2_matriculas as em, escolar2_turmas as et, escolar2_unidades as eu, financeiro_contas as fc WHERE em.vkt_id='$vkt_id' AND em.id='{$_GET['filtro_matricula']}' AND et.id=em.turma_id AND eu.id=et.unidade_id AND fc.id = eu.conta_id"));
	//echo $a;
	echo mysql_error();
}
...
/*else{
		$add2.= "AND et.periodo_letivo_id = '$ultimp_periodo'  	";
}*/
...
$ordem = "ORDER BY m.aluno_id";

$q = mysql_query ($t="SELECT
	DISTINCT(m.id) as id
FROM
	escolar2_matriculas m,
	escolar2_turmas as et,
	escolar2_unidades as eu,
...
if(count($matriculas_id)==0){
	die("<h1>Não há alunos matriculados no(a) {$if[0]} desejado </h1>");
}
$mids=implode(',',$matriculas_id);
/*	 
$q= mysql_query($t="
  SELECT
	  m.*,
  FROM
	  escolar_matriculas m, escolar_alunos as a
  WHERE
	  m.id IN ($mids)
  AND
	  m.aluno_id = a.id 
...
AND
	...
	if($banco->codigo_banco==104){
	   ...
	   include_once("../../boletos/include/funcoes_bb.php");
   }
   //hsbc
   if($banco->codigo_banco==399){
	   $imagem="../../../boletos/imagens/logohsbc.jpg";
	   include_once("../../boletos/include/funcoes_hsbc.php");
   }
   //itau
   if($banco->codigo_banco==409){
	   $imagem="../../../boletos/imagens/logoitau.jpg";
	   include_once("../../boletos/include/funcoes_itau.php");
   }
   //bradesco
   if($banco->codigo_banco==237){
	   $imagem="../../i/l";
	   include_once '../_functions_bradesco.php';
   }
