...
while( $r=mysql_fetch_object($q)){
		...
		SELECT *, DATE_FORMAT(data_registro,'%d/%m/%Y') as data_emitido,DATE_FORMAT(data_vencimento,'%d/%m/%Y') as data_vencimento  FROM financeiro_movimento WHERE doc='".$r->id."'"));
//		echo $b;
$qtd=mysql_result(mysql_query("SELECT COUNT(*) FROM financeiro_movimento WHERE doc='".$r->doc."'"),0);
		$matricula=mysql_fetch_object(mysql_query("SELECT * FROM escolar_matriculas WHERE id='".$r->doc."'"));
