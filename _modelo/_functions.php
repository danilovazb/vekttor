...
//Negociação
function cadastraNegociacao($empreedimento_id,$nome,$entrada,$parcelas_construtora,$parcelas_banco,$valor_construtora,$valor_banco,$obs){
	...
	if(!validaNumero($parcelas_construtora))$erro=1;
	if(!validaNumero($parcelas_banco))$erro=1;
	if(!validaNumero($valor_construtora))$erro=1;
	if(!validaNumero($valor_banco))$erro=1;
	...
	if($erro==0){
		mysql_query($trace="
					...
					nome='".$nome."',
					entrada='".$entrada."',
					parcelas_construtora='".$parcelas_construtora."',
					parcelas_banco='".$parcelas_banco."',
					valor_construtora='".$valor_construtora."',
...
function alteraNegociacao($id,$empreedimento_id,$nome,$entrada,$parcelas_construtora,$parcelas_banco,$valor_construtora,$valor_banco,$obs){
	
	$erro=0;
	if($empreedimento_id>0){
		$r=mysql_fetch_object(mysql_query("SELECT * FROM empreedimento WHERE id='".$empreedimento_id."' LIMIT 1"));
		if(!$r->id>0) return 0;
	}else return 0;
	if($nome!=""){
		$nome=limitaTexto($nome,255);
	...
	if($erro==0){
		mysql_query("
					...
					empreedimento_id='".$empreedimento_id."',
					nome='".$nome."',
					entrada='".$entrada."',
					parcelas_construtora='".$parcelas_construtora."',
					parcelas_banco='".$parcelas_banco."',
...
function excluiNegociacao($id){
	...
	$reserva=mysql_fetch_object(mysql_query("SELECT * FROM reserva WHERE negociacao_id='".$id."' LIMIT 1"));
	if($reserva->id>0)return 0;
	$contrato=mysql_fetch_object(mysql_query("SELECT * FROM contrato WHERE negociacao_id='".$id."' LIMIT 1"));
