...
while($periodo=mysql_fetch_object($s_periodo)){
  ...
  while($escola=mysql_fetch_object($s_escola)){
	...
	while($curso=mysql_fetch_object($s_cursos)){
      ...
      <select name="pago" class="pago" id='pago' style="margin-top:3px;" onchange="location='?tela_id=<?=$_GET[tela_id]?>&pago='+this.value+'&amp;filtro='+document.getElementById('filtro_x').value+'&tipo_matricula='+document.getElementById('tipo_matricula').value+'&busca='+document.getElementById('busca').value">
        <option value="S" <? if($_GET[pago]=='S'){echo "selected='selected'";}?>>Pagos</option>
        <option value="N" <? if($_GET[pago]=='N'){echo "selected='selected'";}?>>N&atilde;o Pago</option>
      </select>
      ...
      <a href="<?php echo $caminho; ?>form.php?curso_id=<?php echo $_GET['curso_id']; ?>&modulo_id=<?php echo $_GET['modulo_id']; ?>" target="carregador" class="mais"></a>    </div>
	...
	?>
        <table cellpadding="0" cellspacing="0" width="100%" id="lista">
		  ...
		  */
			...
			if(strlen($_GET['tipo_matricula'])>1){
				$add2.= " AND m.tipo_matricula = '{$_GET['tipo_matricula']}'   ";
			}
			
            if( $_GET['pago'] ){
                $busca_pago = " AND m.pago='{$_GET[pago]}'";
            } else {
                $busca_pago = " AND m.pago='S'";
            }
...
// echo $t;
            ...
            if( $_GET['ordem'] ){
                ...
                $ordem = "m.escola_id ,m.curso_id , m.modulo_id , m.horario_id, m.sala_id  , a.nome";
            }
	...
	e.nome escola_nome,
	m.sala_id,
	CONCAT(DATE_FORMAT (h.horario_inicio, '%H:%i'), ' às ', DATE_FORMAT (h.horario_fim, '%H:%i') ) horario,
	h.dias_semana,
	a.data_nascimento,
...
WHERE
	...
	$busca_pago 
	 $busca_add
	 $add2
 ORDER BY ".$ordem." ".$_GET['ordem_tipo']." LIMIT ".paginacao_limite($registros,$_GET[pagina],$limitador)) or die ( mysql_error() );
 //echo "<PRE>$t</PRE>";
 			$alteralinha ="0";

				$linha_xls[] ="Sala;Tipo;Pago;MAtricula;Nome Aluno;CPF;Idade;Turma;Dia Selecionado; Valor\n";
           ...
           ?><tr <?php echo $cl; ?> id='<?=$r->matricula_id?>'>
            <td><?=substr($r->tipo,0,1); ?></td>
            <td onclick="o(<?=$r->matricula_id?>)"><?php echo $r->matricula_id; ?></td>
            <td onclick="l(<?=$r->aluno_id?>)"><?php echo $r->aluno_nome; ?></td><td><?php echo $r->aluno_cpf; ?></td><td><?php echo $r->age ?></td><td><?=substr($r->escola_nome,strpos($r->escola_nome,'-')+1,10000)." - ".$r->curso_nome." - ".$r->modulo_nome." - Sala $sala - ".$r->horario." - ".@implode(",",$exibe_dias_horario)?>              <?=$sis['geral']['semana_extenso'][$r->dia_estudar]?>              <?=$valor?></td></tr>
<?php
            }
            ?>	
            </tbody>
...
<div id="rodape" >
	...
	<?php echo $registros; ?> <a href="<?=$file?>">Pagos</a></div>
